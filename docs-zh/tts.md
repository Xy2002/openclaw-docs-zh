...
OpenClaw 可以使用 ElevenLabs、OpenAI 或 Edge TTS 将出站回复转换为音频。只要 OpenClaw 能够发送音频，该功能即可在任何地方使用；在 Telegram 中，它会显示为一个圆形的语音消息气泡。
...
由于 Edge TTS 是一项公开的网络服务，没有公开的 SLA 或配额限制，因此应将其视为尽力而为的服务。如果您需要有保障的限制和支持，请使用 OpenAI 或 ElevenLabs。Microsoft 的 Speech REST API 文档规定每项请求的音频上限为 10 分钟；Edge TTS 并未公布限制，因此可假设其限制与之相似或更低。citeturn0search3

## 可选密钥

如果您希望使用 OpenAI 或 ElevenLabs：
- `ELEVENLABS_API_KEY`（或 `XI_API_KEY`）
- `OPENAI_API_KEY`

Edge TTS **不需要** API 密钥。如果未找到任何 API 密钥，OpenClaw 默认使用 Edge TTS（除非通过 `messages.tts.edge.enabled=false` 禁用）。

如果配置了多个提供商，则优先使用选定的提供商，其他提供商作为备用选项。自动摘要使用已配置的 `summaryModel`（或 `agents.defaults.model.primary`），因此如果您启用摘要功能，该提供商也必须经过身份验证。
...
- `provider`: `"elevenlabs"`, `"openai"` 或 `"edge"`（备用自动生效）。
...
- `elevenlabs.baseUrl`: 转换 ElevenLabs API 基础 URL。
...
启用后，模型可以发出 `[[tts:...]]` 指令来覆盖单个回复的语音，还可以发出一个可选的 `[[tts:text]]...[[/tts:text]]` 块，以提供表情标签（如笑声、唱歌提示等），这些标签只应在音频中出现。
...
- **Telegram**: Opus 语音消息（来自 ElevenLabs 的 `opus_48000_64`，来自 OpenAI 的 `opus`）。
  - 48kHz / 64kbps 是语音消息的良好折衷方案，也是圆形气泡所必需的。
...
- **Edge TTS**: 使用 `edge.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`）。
  ...
  - Telegram `sendVoice` 接受 OGG/MP3/M4A；如果您需要有保证的 Opus 语音消息，请使用 OpenAI/ElevenLabs。citeturn1search1
  - 如果配置的 Edge 输出格式失败，OpenClaw 会尝试使用 MP3。
...
- 跳过非常短的回复（少于 10 个字符）。
- 在启用的情况下，使用 `agents.defaults.model.primary`（或 `summaryModel`）对长回复进行摘要。
- 将生成的音频附加到回复上。

如果回复超过 `maxLength` 且摘要功能关闭（或摘要模型没有 API 密钥），则会跳过音频，并发送普通的文本回复。
...
Discord 注意： `/tts` 是 Discord 内置命令，因此 OpenClaw 在那里注册了 `/voice` 作为原生命令。文本 `/tts ...` 仍然有效。
...
`tts` 工具将文本转换为语音，并返回一个 `MEDIA:` 路径。当结果与 Telegram 兼容时，该工具会包含 `[[audio_as_voice]]`，以便 Telegram 发送语音气泡。
