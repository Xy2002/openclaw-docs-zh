...
---
使用基础设施即代码在 Render 上部署 OpenClaw。随附的 `render.yaml` 蓝图以声明式方式定义了您的整个堆栈：服务、磁盘、环境变量，让您只需单击即可完成部署，并将基础设施版本与代码版本同步管理。

## 先决条件

- 一个 [Render 账户](https://render.com)（提供免费层级）
- 从您首选的 [模型提供商](/providers) 获取的 API 密钥
...
1. 从此仓库根目录的 `render.yaml` 蓝图创建一个新的 Render 服务。
...
| 方案 | 自动关闭 | 磁盘 | 适合场景 |
|------|-----------|------|----------|
| 免费 | 空闲 15 分钟后关闭 | 不可用 | 测试、演示 |
| 初学者 | 永不关闭 | 1GB+ | 个人使用、小型团队 |
| 标准+ | 永不关闭 | 1GB+ | 生产环境、多渠道 |

蓝图默认使用 `starter`。要使用免费层级，请在您的分叉副本中更改 `plan: free` 的 `render.yaml`（但请注意：由于没有持久化磁盘，每次部署时配置都会重置）。
...
在“仪表板 → 您的服务 → 日志”中查看实时日志。可按以下内容筛选：
- 构建日志（Docker 镜像创建）
- 部署日志（服务启动）
- 运行时日志（应用程序输出）
...
对于 OpenClaw，通常垂直扩展已足够。水平扩展需要使用粘性会话或外部状态管理。
...
- 缺少 `SETUP_PASSWORD` — 蓝图会提示输入，但请确认已正确设置
- 端口不匹配 — 确保 `PORT=8080` 与 Dockerfile 中暴露的端口一致

### 冷启动缓慢（免费层级）

免费层级的服务在空闲 15 分钟后会关闭。关闭后首次请求需要几秒钟来启动容器。升级到 Starter 方案可实现始终在线。
