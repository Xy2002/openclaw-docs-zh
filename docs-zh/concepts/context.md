...
- `/context detail` → 更深入的细分：按文件、按工具模式大小、按技能条目大小，以及系统提示的大小。
- `/usage tokens` → 在普通回复中附加每条回复的用量脚注。
- `/compact` → 将较旧的历史摘要为一条紧凑条目，以释放窗口空间。
...
技能说明默认 *不包含在内*。模型预计会在 **需要时** `read` 技能的 `SKILL.md`。
...
- **内联快捷方式**（仅限白名单发件人）：普通消息中某些 `/...` 标记可以立即执行（例如，“hey /status”），并在模型看到剩余文本之前被剥离。
...
跨消息持久化的内容取决于机制：
- **常规历史记录** 会保留在会话转录中，直到根据策略被压缩或修剪。
- **压缩** 会将摘要保留在转录中，并保持最近的消息完好无损。
- **修剪** 会从一次运行的 *内存中* 提示中移除旧的工具结果，但不会重写转录。
