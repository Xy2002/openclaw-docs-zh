...
- 为获得最佳效果，应将 `ttl` 与您的模型 `cacheControlTtl` 匹配。
- 在修剪后，TTL 窗口会重置，因此后续请求可在 `ttl` 再次到期前继续使用缓存。

## 智能默认设置（Anthropic）
- **OAuth 或 setup-token** 配置文件：启用 `cache-ttl` 修剪，并将心跳设置为 `1h`。
- **API 密钥** 配置文件：启用 `cache-ttl` 修剪，将心跳设置为 `30m`，并将 Anthropic 模型上的 `cacheControlTtl` 默认设置为 `1h`。
- 如果您显式设置了这些值，OpenClaw**不会**覆盖它们。

## 改进内容（成本 + 缓存行为）
- **为何要修剪：** Anthropic 提示词缓存仅在 TTL 内有效。如果会话在 TTL 后处于空闲状态，下一次请求将重新缓存整个提示词，除非您事先进行修剪。
- **哪些成本降低：** 修剪可减少 TTL 到期后首次请求的 **cacheWrite** 大小。
- **TTL 重置的重要性：** 修剪一旦运行，缓存窗口就会重置，因此后续请求可以复用新缓存的提示词，而无需再次缓存完整的历史记录。
- **修剪不会做什么：** 修剪不会增加令牌数量或导致“双重”成本；它只改变 TTL 后首次请求中被缓存的内容。
