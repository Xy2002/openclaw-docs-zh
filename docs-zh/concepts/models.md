...
如果设置了 `agents.defaults.models`，它将成为 `/model` 以及会话覆盖的 **白名单**。当用户选择的模型不在该白名单中，OpenClaw 会返回：
...
- `/model <#>` 从该选择器中选择。
- `/model status` 是详细视图（身份验证候选以及在配置时的提供者端点 `baseUrl` + `api` 模式）。
...
- 如果省略提供者，OpenClaw 会将输入视为 **默认提供者** 的别名或模型（仅在模型 ID 中没有 `/` 时有效）。
...
使用 `--check` 进行自动化（凭据缺失或过期时退出 `1`，即将过期时退出 `2`）。

Anthropic 推荐的身份验证方式是 Claude Code CLI setup-token（可在任何地方运行；必要时粘贴到网关主机上）：
...
探测需要 OpenRouter API 密钥（来自身份验证配置文件或 `OPENROUTER_API_KEY`）。如果没有密钥，使用 `--no-probe` 仅列出候选模型。
...
`models.providers` 中的自定义提供者会写入 `models.json`，位于代理目录下（默认 `~/.openclaw/agents/<agentId>/models.json`）。此文件默认会被合并，除非 `models.mode` 设置为 `replace`。
