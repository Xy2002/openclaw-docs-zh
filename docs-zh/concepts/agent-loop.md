...
代理循环是代理的完整“真实”运行过程：输入 → 上下文组装 → 模型推理 → 工具执行 → 流式回复 → 持久化。它是将消息转化为行动和最终回复，并保持会话状态一致的权威路径。

在OpenClaw中，一个循环代表每个会话的一次序列化运行，它会在模型思考、调用工具并流式输出时发出生命周期和流事件。本文档详细说明了这一真实循环是如何端到端连接起来的。

## 入口点
- 网关RPC：`agent` 和 `agent.wait`。
- CLI：`agent` 命令。
...
## 队列与并发
- 每个会话键（会话车道）内的运行按顺序执行，也可选择通过全局车道执行。
...
## 提示组装与系统提示
- 系统提示由 OpenClaw 的基础提示、技能提示、引导上下文以及每轮覆盖构建而成。
- 强制执行特定于模型的限制和压缩预留标记。
...
钩子 API 和注册详情请参见 [插件](/plugin#plugin-hooks)。

## 流式传输 + 部分回复
- 助手增量从 pi-agent-core 流式传输，并作为 `assistant` 事件发出。
- 块状流式传输可以在 `text_end` 或 `message_end` 上发出部分回复。
- 推理流式传输可以作为单独的流发出，也可以作为块状回复发出。
- 参见 [流式传输](/concepts/streaming)，了解分块和块状回复的行为。
...
- 最终有效载荷由以下内容组成：
  ...
  - 内联工具摘要（当启用详细模式且允许时）
  - 当模型出错时的助手错误文本
- `NO_REPLY` 被视为静默标记，并从传出有效载荷中过滤掉。
- 消息工具的重复项从最终有效载荷列表中移除。
- 如果没有可渲染的有效载荷剩余且工具出错，则发出后备工具错误回复
  （除非消息工具已经发送了用户可见的回复）。

## 压缩 + 重试
- 自动压缩会发出 `compaction` 流事件，并可能触发重试。
- 在重试时，内存缓冲区和工具摘要会被重置，以避免重复输出。
- 压缩管道请参见 [压缩](/concepts/compaction)。
...
- 在 **生命周期结束/错误** 时，会发出聊天 `final`。

## 超时
- 默认 `agent.wait`：30秒（仅等待）。`timeoutMs` 参数可覆盖。
- 代理运行时：默认 `agents.defaults.timeoutSeconds` 600秒；在 `runEmbeddedPiAgent` 中强制执行中止计时器。
