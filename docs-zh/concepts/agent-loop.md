...
- 获取会话写锁；在开始流式传输之前，打开并准备 `SessionManager`。
...
- 关于分块和块式回复行为，请参见 [流式传输](/concepts/streaming)。
...
- 最终负载由以下内容组成：
  ...
  - 内联工具摘要（当启用详细模式且允许时）
  - 当模型出错时的助手错误文本
- `NO_REPLY` 被视为静默标记，并从传出负载中过滤掉。
- 消息工具的重复项从最终负载列表中移除。
- 如果没有可渲染的负载剩余且工具出错，则发出回退工具错误回复
  （除非消息工具已经发送了用户可见的回复）。
...
## 超时
- 默认超时时间为 `agent.wait`：30秒（仅等待）。可通过 `timeoutMs` 参数覆盖。
- 代理运行时默认为 `agents.defaults.timeoutSeconds`：600秒；在 `runEmbeddedPiAgent` 中断定时器中强制执行。
