...
## 初学者入门（2分钟）
OpenClaw“驻扎”在你自己的消息账户中，不存在单独的 WhatsApp 机器人用户。
如果**你**在某个群组中，OpenClaw 就能看到该群组并在此群组中作出回应。
...
> - **群组访问权限**由 `*.groupPolicy` 加上白名单（`*.groups`、`*.groupAllowFrom`）控制。
...
| 只有你可以触发群组中的响应 | `groupPolicy: "allowlist"`、`groupAllowFrom: ["+1555..."]` |

## 会话密钥
- 群组会话使用 `agent:<agentId>:<channel>:group:<id>` 会话密钥（房间/频道使用 `agent:<agentId>:<channel>:channel:<id>`）。
- Telegram 论坛话题在群组 ID 中添加 `:topic:<threadId>`，因此每个话题都有自己的会话。
- 直接聊天使用主会话（或根据配置为每发送者会话）。
- 群组会话跳过心跳检测。
...
原因：在单代理模式下，私信通常进入**主**会话密钥（`agent:main:main`），而群组始终使用**非主**会话密钥（`agent:main:<channel>:group:<id>`）。如果你启用沙盒功能（`mode: "non-main"`），这些群组会话将在 Docker 中运行，而你的主私信会话则保留在主机上。

这样，你有一个代理“大脑”（共享工作空间 + 内存），但有两种执行姿态：
- **私信**：完整工具集（主机）
- **群组**：沙盒 + 有限工具集（Docker）

> 如果你需要真正独立的工作空间/角色身份（“个人”和“公共”绝不能混杂），请使用第二个代理 + 绑定。参见 [多代理路由](/concepts/multi-agent)。
...
想要“群组只能看到文件夹 X”，而不是“无主机访问权限”？保留 `workspaceAccess: "none"`，只将白名单路径挂载到沙盒中：
...
- 调试工具被阻止的原因：[沙盒 vs 工具策略 vs 提升权限](/gateway/sandbox-vs-tool-policy-vs-elevated)
...
- Matrix：白名单使用 `channels.matrix.groups`（房间 ID、别名或名称）。使用 `channels.matrix.groupAllowFrom` 来限制发件人；还支持按房间的 `users` 白名单。
...
2) 群组白名单（`*.groups`、`*.groupAllowFrom`、特定渠道白名单）
...
- 提供显式提及的平台仍会通过；模式只是后备方案。
- 按代理覆盖：`agents.list[].groupChat.mentionPatterns`（当多个代理共享一个群组时很有用）。
- 提及 gating 仅在可以检测提及时强制执行（已配置原生提及或 `mentionPatterns`）。
- Discord 的默认设置位于 `channels.discord.guilds."*"` 中（可按公会/频道覆盖）。
- 群组历史上下文在各渠道中统一封装，且仅适用于**待处理消息**（因提及 gating 而被跳过的消息）；使用 `messages.groupChat.historyLimit` 作为全局默认，使用 `channels.<channel>.historyLimit`（或 `channels.<channel>.accounts.*.historyLimit`）进行覆盖。设置 `0` 可以禁用。

## 群组/频道工具限制（可选）
某些渠道配置支持限制**特定群组/房间/频道内**可用的工具。

- `tools`：允许/禁止整个群组使用某些工具。
- `toolsBySender`：群组内的按发送者覆盖（键为发送者 ID/用户名/电子邮件/电话号码，具体取决于渠道）。使用 `"*"` 作为通配符。
...
- 某些渠道对房间/频道的嵌套有不同的规定（例如，Discord `guilds.*.channels.*`、Slack `channels.*`、MS Teams `teams.*.channels.*`）。
...
## iMessage 特殊说明
- 在路由或白名单中优先使用 `chat_id:<id>`。
- 列表聊天： `imsg chats --limit 20`。
- 群组回复始终返回到同一个 `chat_id`。

## WhatsApp 特殊说明
有关 WhatsApp 独有的行为（历史注入、提及处理细节），请参阅 [群组消息](/concepts/group-messages)。
