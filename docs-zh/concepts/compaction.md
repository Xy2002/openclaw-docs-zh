...
## 什么是压缩
压缩会将较早的对话内容**总结为一条紧凑的摘要条目**，同时保留最新的消息不变。该摘要会存储在会话历史中，因此未来的请求将使用：
- 压缩摘要
- 压缩点之后的最新消息

压缩结果会在会话的 JSONL 历史中**持久保存**。
...
在压缩之前，OpenClaw可以执行一轮**静默内存刷新**，将持久性笔记存储到磁盘。有关详情和配置，请参阅[内存](/concepts/memory)。
...
## 上下文窗口来源
上下文窗口是特定于模型的。OpenClaw会根据已配置提供商目录中的模型定义来确定这些限制。

## 压缩与修剪的区别
- **压缩**：对对话进行总结并**持久化**到 JSONL 中。
