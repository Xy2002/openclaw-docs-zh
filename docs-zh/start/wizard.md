...
推荐：设置 Brave Search API 密钥，以便代理可以使用 `web_search`（`web_fetch` 无需密钥即可使用）。最简单的方法：`openclaw configure --section web`，它会存储 `tools.web.search.apiKey`。文档：[Web 工具](/tools/web)。

## QuickStart 与高级

向导首先提供 **QuickStart**（默认设置）和 **高级**（完全控制）两种模式。
...
- Tailscale 暴露 **关闭**
- Telegram 和 WhatsApp 私信默认为 **白名单**（系统会提示您输入电话号码）
...
3) **工作区**
   ...
   - 为代理引导仪式所需的工作区文件提供种子数据。
   - 完整的工作区布局 + 备份指南：[代理工作区](/concepts/agent-workspace)

4) **网关**
   - 端口、绑定、认证模式、Tailscale 暴露。
   - 认证建议：即使在环回模式下也应保持 **Token**，以便本地 WS 客户端必须进行认证。
   - 仅在您完全信任每个本地进程时才禁用认证。
   - 非环回绑定仍然需要认证。
...
5) **通道**
   ...
   - 私信安全：默认为配对模式。首次私信会发送一个代码；可通过 `openclaw pairing approve <channel> <code>` 批准，或使用白名单。

6) **守护进程安装**
   - macOS：LaunchAgent
     - 需要登录的用户会话；对于无头环境，请使用自定义 LaunchDaemon（未随附）。
...
8) **技能（推荐）**
   - 读取可用技能并检查所需条件。
   - 让您选择节点管理器：**npm / pnpm**（不推荐使用 bun）。
   - 安装可选依赖项（部分依赖项在 macOS 上使用 Homebrew 安装）。

9) **完成**
   - 总结 + 下一步，包括 iOS/Android/macOS 应用程序以获取更多功能。
...
使用 `openclaw agents add <name>` 创建一个具有独立工作区、会话和认证档案的单独代理。如果不使用 `--workspace`，将启动向导。
...
网关通过 RPC 暴露向导流程（`wizard.start`、`wizard.next`、`wizard.cancel`、`wizard.status`）。客户端（macOS 应用、控制 UI）可以在不重新实现入门逻辑的情况下呈现各个步骤。
...
- 通道白名单（Slack/Discord/Matrix/Microsoft Teams）在您在提示中选择加入时生效（名称尽可能解析为 ID）。
...
某些通道以插件形式提供。当您在入门过程中选择其中一个时，向导会提示您安装它（通过 npm 或本地路径），然后再进行配置。
