...
## 简要说明
- **在代码库外进行个性化配置：** `~/.openclaw/workspace`（工作区）+ `~/.openclaw/openclaw.json`（配置）。
- **稳定的工作流：** 安装 macOS 应用程序；让其运行捆绑的 Gateway。
...
如果你尚未全局安装，可通过 `pnpm openclaw setup` 来运行。
