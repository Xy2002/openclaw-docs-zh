...
目标：每个消息传递连接器都作为一个插件（内置或外部）使用一套稳定的 API。任何插件都不直接从 `src/**` 导入。所有依赖项都通过 SDK 或运行时进行管理。

## 为何现在进行重构
- 当前的连接器混用了多种模式：直接导入核心、仅提供 dist 的桥接模块以及自定义辅助工具。
- 这种混合模式导致升级脆弱，并阻碍了干净的外部插件接口的形成。
...
- 将 `api.runtime` 添加到 `OpenClawPluginApi`，并提供上述接口。
- 在过渡期内保留现有导入，并发出弃用警告。
...
### 阶段 4：iMessage 插件化
- 将 iMessage 迁移到 `extensions/imessage`。
- 用 `api.runtime` 替代直接的核心调用。
- 保持配置键、CLI 行为和文档不变。

### 阶段 5：强制执行
- 添加 lint 规则 / CI 检查：禁止从 `src/**` 中导入 `extensions/**`。
- 添加插件 SDK/运行时版本兼容性检查（运行时 + SDK 采用语义版本控制）。

## 兼容性和版本管理
- SDK：采用语义版本控制，公开发布，并记录变更。
- 运行时：随核心版本进行版本化管理。添加 `api.runtime.version`。
- 插件需声明所需的运行时范围（例如 `openclawRuntime: ">=2026.2.0"`）。

## 测试策略
- 适配器级别的单元测试：使用真实的核心实现来测试运行时函数。
- 每个插件的 Golden 测试：确保行为无漂移（路由、配对、白名单、提及控制）。
- 在 CI 中使用单个端到端插件示例：安装 + 运行 + 烟雾测试。
...
## 成功标准
- 所有通道连接器都成为使用 SDK + 运行时的插件。
- 不再从 `src/**` 中导入 `extensions/**`。
- 新的连接器模板仅依赖于 SDK + 运行时。
- 外部插件无需访问核心源码即可开发和更新。
