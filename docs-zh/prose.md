...
OpenProse 是一种便携式、以 Markdown 优先的工作流格式，用于编排 AI 会话。在 OpenClaw 中，它以插件形式提供，安装一个 OpenProse 技能包以及一条 `/prose` 斜杠命令。程序存储在 `.prose` 文件中，并可通过显式控制流生成多个子代理。
