...
OpenClaw 的设计以一个拥有会话状态的单一 **网关进程** 为核心。
...
OpenClaw 通过两层机制来持久化会话：

1) **会话存储（`sessions.json`）**
   - 键值映射：`sessionKey -> SessionEntry`
   - 数据量小、可修改、安全地编辑（或删除）条目
...
- **每日重置**（网关主机当地时间凌晨 4:00 默认设置）在重置边界之后的第一条消息创建一个新的 `sessionId`。
...
存储可以安全地编辑，但网关具有权威性：它可能会在会话运行过程中重写或重新填充条目。
...
- `custom_message`：注入的扩展消息，*会* 进入模型上下文（可在 UI 中隐藏）
...
压缩是 **持久的**（不同于会话修剪）。有关更多信息，请参阅 [/concepts/session-pruning](/concepts/session-pruning)。
...
目标：在自动压缩发生之前，运行一个静默的代理回合，将持久状态写入磁盘（例如，代理工作区中的 `memory/YYYY-MM-DD.md`），以便压缩无法擦除关键上下文。
...
- 当会话工作区为只读时（`workspaceAccess: "ro"` 或 `"none"`），刷新会被跳过。
...
- 存储与对话记录是否不匹配？请确认网关主机以及从 `openclaw status` 获取的存储路径。
- 压缩过于频繁？请检查：
  - 模型上下文窗口（太小）
  - 压缩设置（如果 `reserveTokens` 对于模型窗口来说过高，可能导致更早的压缩）
  - 工具结果膨胀：启用/调整会话修剪
- 静默回合是否泄漏？请确认回复是否以 `NO_REPLY`（确切标记）开头，并且您使用的是包含流式传输抑制修复的构建版本。
