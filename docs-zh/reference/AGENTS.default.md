...
- **sag** — ElevenLabs 语音合成，采用 macOS 风格的 say UX；默认流式传输至扬声器。
- **Sonos CLI** — 从脚本控制 Sonos 扬声器（发现/状态/播放/音量/分组）。
- **blucli** — 从脚本控制、分组并自动化 BluOS 播放器。
- **OpenHue CLI** — 用于场景和自动化控制的飞利浦 Hue 照明控制器。
- **OpenAI Whisper** — 本地语音转文本功能，适用于快速听写和语音邮件转录。
- **Gemini CLI** — 从终端调用 Google Gemini 模型，实现快速问答。
- **bird** — X/Twitter CLI，无需浏览器即可发推文、回复、阅读线程并进行搜索。
