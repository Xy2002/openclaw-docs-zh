...
从仓库根目录使用 `pnpm`（Node 22+）。在打标签/发布之前，确保工作树保持干净。

## 运营商触发
当运营商说“发布”时，立即执行此预检（除非遇到阻碍，否则无需额外提问）：
...
- [ ] 如果依赖项发生变化，运行 `pnpm install`，以确保 `pnpm-lock.yaml` 为最新状态。
...
- [ ] 可选：在构建后运行 `npm pack --pack-destination /tmp`；检查 tarball 内容并将其保存以备 GitHub 发布之用（**不要**将其提交）。

3) **变更日志与文档**
- [ ] 使用面向用户的亮点更新 `CHANGELOG.md`（如果文件缺失则创建）；条目应按版本严格降序排列。
...
- [ ] `OPENCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`（Docker 安装烟雾测试，快速路径；发布前必需）
  - 如果已知上一个 npm 发布存在严重问题，为 preinstall 步骤设置 `OPENCLAW_INSTALL_SMOKE_PREVIOUS=<last-good-version>` 或 `OPENCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1`。
- [ ] （可选）完整安装程序烟雾测试（增加非 root + CLI 覆盖率）：`pnpm test:install:smoke`
- [ ] （可选）安装程序端到端测试（Docker，运行 `curl -fsSL https://openclaw.bot/install.sh | bash`，完成注册，然后执行实际工具调用）：
...
- **npm pack/publish 卡住或生成超大 tarball**：macOS 应用程序包在 `dist/OpenClaw.app`（以及发布 zip）中被扫入软件包。通过白名单发布内容来修复：`package.json` `files`（包括 dist 子目录、文档、技能；排除应用程序包）。使用 `npm pack --dry-run` 确认 `dist/OpenClaw.app` 未列出。
...
- [ ] 为 `vX.Y.Z` 创建/刷新 GitHub 发布，**标题为 `openclaw X.Y.Z`**（不仅仅是标签）；正文应包含该版本的**完整**变更日志部分（亮点 + 变更 + 修复），内联显示（无裸链接），并且**不得在正文中重复标题**。
...
发布说明还必须指出**新的可选捆绑插件**，这些插件**默认情况下不启用**（例如，`tlon`）。
