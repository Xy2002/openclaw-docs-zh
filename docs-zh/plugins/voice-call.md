...
通过插件为 OpenClaw 提供语音通话功能。支持出站通知以及具有入站策略的多轮对话。
...
语音通话使用核心 `messages.tts` 配置（OpenAI 或 ElevenLabs）来在通话中进行流式语音播放。您可以在插件配置中以 **相同结构** 覆盖它——它会与 `messages.tts` 进行深度合并。
...
注意事项：
- **边缘 TTS 对于语音通话被忽略**（电信音频需要 PCM；Edge 输出不可靠）。
- 当启用 Twilio 媒体流时使用核心 TTS；否则，通话将回退到提供商的原生语音。
