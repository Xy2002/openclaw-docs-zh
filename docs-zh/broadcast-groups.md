...
广播群组允许多个代理同时处理并响应同一条消息。这使您能够创建专门的代理团队，在同一个 WhatsApp 群组或私信中协同工作——所有这些都使用同一个电话号码。

当前范围：仅限 **WhatsApp**（Web 渠道）。

广播群组在渠道白名单和群组激活规则之后进行评估。在 WhatsApp 群组中，这意味着当 OpenClaw 通常会回复时（例如：根据您的群组设置，在被提及时），广播就会发生。
...
添加一个顶级 `broadcast` 部分（与 `bindings` 相邻）。键是 WhatsApp 对等 ID：
...
注意：广播群组不会绕过渠道白名单或群组激活规则（提及/命令等）。它们只改变在消息符合处理条件时 *哪些代理会运行*。
...
在群组 `120363403215116621@g.us` 中，有代理 `["alfred", "baerbel"]`：
...
### 只有一个代理响应
**原因：** 对等 ID 可能存在于 `bindings` 中，但不在 `broadcast` 中。
...
- docs-checker：“缺少 `process_data` 函数的文档字符串”
