...
添加一个顶级 `broadcast` 部分（与 `bindings` 并列）。键是 WhatsApp 对等 ID：
...
注意：广播群组不会绕过渠道白名单或群组激活规则（提及/命令等）。它们只改变当消息符合处理条件时 *哪些代理会运行*。
