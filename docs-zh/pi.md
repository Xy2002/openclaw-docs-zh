...
6. **模式归一化**：针对 Gemini/OpenAI 的特殊性清理模式
...
这确保了 OpenClaw 的策略过滤、沙箱集成以及扩展工具集在不同提供商之间保持一致。

## 系统提示构建

系统提示在 `buildAgentSystemPrompt()` (`system-prompt.ts`) 中构建。它组装了一个包含以下部分的完整提示：工具、工具调用风格、OpenClaw CLI 参考、技能、文档、工作区、沙箱、消息传递、回复标签、语音、静默回复、心跳、运行时元数据，以及启用时的记忆和反应，还有可选的上下文文件和额外的系统提示内容。对于子代理使用的最小提示模式，各部分会被裁剪。
...
会话是具有树形结构（id/parentId 链接）的 JSONL 文件。Pi 的 `SessionManager` 负责持久化：
...
`limitHistoryTurns()` 根据频道类型（私聊 vs 群聊）修剪对话历史。
...
`pi-embedded-helpers.ts` 对错误进行分类，以便进行适当的处理：
...
4. **流式处理器复杂性**：`subscribeEmbeddedPiSession` 已经变得庞大
