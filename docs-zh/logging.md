...
本页面将介绍日志的存储位置、读取方式，以及如何配置日志级别和格式。

## 日志的存储位置

默认情况下，网关会将滚动日志文件写入以下路径：
...
在 JSON 模式下，CLI 输出带有 `type` 标签的对象：
...
控制 UI 的 **日志** 选项卡使用 `logs.tail` 尾部跟踪同一文件。有关如何打开它的信息，请参阅 [/web/control-ui](/web/control-ui)。
...
日志文件中的每一行都是一个 JSON 对象。CLI 和控制 UI 会解析这些条目，以呈现结构化输出（时间、级别、子系统、消息）。
...
所有日志配置都位于 `logging` 下的 `~/.openclaw/openclaw.json` 中。
...
- `diagnostic.heartbeat`：汇总计数器（webhooks/队列/会话）。

### 启用诊断（无导出器）

如果您希望插件或自定义接收器可以使用诊断事件，请使用此方法：
