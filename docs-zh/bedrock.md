...
OpenClaw 可以通过 pi‑ai 的 **Bedrock Converse** 流式传输提供商使用 **Amazon Bedrock** 模型。Bedrock 身份验证使用 **AWS SDK 默认凭证链**，而非 API 密钥。
...
如果检测到 AWS 凭证，OpenClaw 可以自动发现支持 **流式传输** 和 **文本输出** 的 Bedrock 模型。发现过程使用 `bedrock:ListFoundationModels`，并进行缓存（默认缓存时间为 1 小时）。
...
- `defaultContextWindow`（默认：`32000`）和 `defaultMaxTokens`（默认：`4096`）用于已发现的模型；如果您了解模型限制，可以进行覆盖。
