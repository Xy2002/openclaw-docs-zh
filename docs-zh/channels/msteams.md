...
## 插件要求
Microsoft Teams以插件形式提供，不包含在核心安装中。
...
- 当`groupPolicy="allowlist"`和团队白名单存在时，仅接受列出的团队/频道（受提及限制）。
...
4. 将Teams应用上传/安装到团队中（或用于DM的个人范围）。
...
> **弃用通知：** 自2025年7月31日之后，不再创建新的多租户机器人。对于新机器人，请使用**单租户**。
...
2. 将**消息端点**设置为您的webhook URL：
   ...
   - 本地开发：使用隧道（参见[本地开发](#local-development-tunneling)下方）
...
3. 转到**应用功能** → **机器人**
4. 选择**手动输入机器人ID**并粘贴您的Azure Bot App ID
5. 检查范围：**个人**、**团队**、**群聊**
6. 单击**分发** → **下载应用包**
...
1. 在Azure门户 → 您的Azure Bot资源 → **在Web Chat中测试**
2. 发送一条消息 - 您应该看到响应
...
- DM历史可以通过`channels.msteams.dmHistoryLimit`（用户开启）来限制。每用户覆盖：`channels.msteams.dms["<user_id>"].historyLimit`。
...
### 清单注意事项（必填字段）
- `bots[].botId`必须匹配Azure Bot App ID。
- `webApplicationInfo.id`必须匹配Azure Bot App ID。
...
6. **完全退出并重新启动Teams**（不仅仅是关闭窗口）以清除缓存的应用元数据
...
**底线：** RSC用于实时监听；Graph API用于历史访问。要在离线时补上错过的消息，您需要具有`ChannelMessage.Read.All`的Graph API（需要管理员同意）。
...
OpenClaw通过快速返回并主动发送回复来处理这种情况，但非常慢的响应仍可能导致问题。
...
- `channels.msteams.requireMention`：在频道/群组中强制要求@提及（默认为真）。
...
- `channels.msteams.teams.<teamId>.tools`：默认的按团队的每发送方工具政策覆盖（`allow`/`deny`/`alsoAllow`），在缺少频道覆盖时使用。
...
- `deny`：按频道的每发送方工具政策覆盖（`channels.msteams.sharePointSiteId`）。
- `alsoAllow`：用于群聊/频道中文件上传的SharePoint站点ID（参见[在群聊中发送文件](#sending-files-in-group-chats)）。
...
- `top-level`在帖子风格的频道中 → 回复会作为单独的顶级帖子出现，而不是在线程中
...
**当前限制：**
- **DMs：** 图片和文件附件通过Teams机器人文件API工作。
- **频道/群组：** 附件存储在M365存储中（SharePoint/OneDrive）。Webhook负载仅包含HTML占位符，不包含实际的文件字节。**需要Graph API权限**才能下载频道附件。
...
按用户共享更安全，因为只有聊天参与者才能访问文件。如果缺少`Chat.Read.All`权限，机器人会回退到组织范围内的共享。
...
| 群聊 + 文件 + 缺少`sharePointSiteId` | 尝试OneDrive上传（可能失败），仅发送文本 |
| 个人聊天 + 文件 | FileConsentCard流程（无需SharePoint即可工作） |
| 任何上下文 + 图片 | Base64编码的内联（无需SharePoint即可工作） |
...
## 投票（自适应卡片）
OpenClaw将Teams投票作为自适应卡片发送（没有原生的Teams投票API）。
...
`card`参数接受自适应卡片JSON对象。当`card`提供时，消息文本是可选。
...
有关卡片模式和示例，请参见[自适应卡片文档](https://adaptivecards.io/)。有关目标格式的详细信息，请参见[目标格式](#target-formats)下方。
...
- **频道中看不到图片：** 缺少Graph权限或管理员同意。重新安装Teams应用并完全退出/重新打开Teams。
...
- **“webApplicationInfo.Id已在使用中”：** 应用仍在其他团队/聊天中安装。先找到并卸载，或等待5-10分钟以待传播。
