...
状态：通过 grammY 实现的机器人私聊和群组功能已进入生产就绪阶段。默认采用长轮询；Webhook 为可选配置。
...
3) 启动网关。
4) 默认情况下，私聊访问通过配对实现；首次联系时需批准配对码。
...
多账户支持：使用 `channels.telegram.accounts` 配置每个账户的令牌，并可选使用 `name`。共享模式请参见 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。
...
### 群组消息可见性（隐私模式）
Telegram 机器人默认启用 **隐私模式**，限制其接收的群组消息范围。
如果您的机器人必须查看 *所有* 群组消息，您有两个选择：
- 使用 `/setprivacy` 禁用隐私模式 **或**
- 将机器人设为群组 **管理员**（管理员机器人可接收所有消息）。

**注意：** 当您切换隐私模式时，Telegram 要求从每个群组中移除并重新添加机器人，才能使更改生效。

### 群组权限（管理员权限）
管理员状态在群组内部（Telegram UI）设置。管理员机器人始终接收所有群组消息，因此如果您需要完全可见性，请使用管理员权限。
...
- Telegram 机器人 API 不支持已读回执；没有 `sendReadReceipts` 选项。
...
- 如果 Telegram 拒绝 HTML 负载，OpenClaw 会以纯文本形式重试同一消息。
...
注意事项：
- 自定义命令仅为 **菜单条目**；除非您在其他地方处理，否则 OpenClaw 不会实现它们。
- 命令名称经过标准化（去除前导 `/`，转换为小写），且必须符合 `a-z`、`0-9`、`_`（1–32 个字符）的要求。
...
- Telegram 机器人 API 请求在 `channels.telegram.timeoutSeconds` 后超时（默认通过 grammY 设置为 500）。设置更低值以避免长时间挂起。
...
- DM 历史可通过 `channels.telegram.dmHistoryLimit` 限制（用户可调整）。每用户覆盖：`channels.telegram.dms["<user_id>"].historyLimit`。
...
论坛主题继承其父群组的配置（allowFrom、requireMention、skills、prompts），除非您在 `channels.telegram.groups.<groupId>.topics.<topicId>` 下添加针对特定主题的覆盖。
...
将群组中的任何消息转发到 Telegram 上的 `@userinfobot` 或 `@getidsbot`，即可查看聊天 ID（如 `-1001234567890` 的负数）。
...
这种情况发生在：
- 当群组升级为超级群组时，Telegram 发出 `migrate_to_chat_id`（聊天 ID 更改）。OpenClaw 可以自动迁移 `channels.telegram.groups`。
...
- 发送打字指示器和回复时使用 `message_thread_id`，使回复保持在主题内。
- 通用主题（线程 ID `1`）比较特殊：发送消息时省略 `message_thread_id`（Telegram 会拒绝），但打字指示器仍包含它。
- 在模板上下文中公开 `MessageThreadId` 和 `IsForum`，用于路由/模板化。
- 特定主题的配置可在 `channels.telegram.groups.<chatId>.topics.<threadId>` 下获得（技能、白名单、自动回复、系统提示、禁用）。
- 主题配置继承群组设置（requireMention、白名单、技能、提示、启用），除非在主题层面被覆盖。
...
- `group` — 仅限群组（阻止 DM 目标）
- `all` — DM 和群组
- `allowlist` — DM 和群组，但仅允许 `allowFrom`/`groupAllowFrom` 允许的发送者（与控制命令相同的规则）
...
Telegram 的能力可以在两个级别上配置（上述显示的是对象形式；旧版字符串数组仍受支持）：

- `channels.telegram.capabilities`: 全局默认能力配置应用于所有 Telegram 账户，除非被覆盖。
- `channels.telegram.accounts.<account>.capabilities`: 每账户的能力，覆盖该特定账户的全局默认值。

当所有 Telegram 机器人/账户的行为应相同时，使用全局设置。当不同机器人需要不同行为时（例如，一个账户只处理 DM，而另一个账户被允许进入群组），使用每账户配置。
## 访问控制（DMs + 群组）

### DM 访问
- 默认：`channels.telegram.dmPolicy = "pairing"`。未知发送者会收到配对代码；消息在批准前会被忽略（代码 1 小时后失效）。
- 批准方式：
  - `openclaw pairing list telegram`
  - `openclaw pairing approve telegram <CODE>`
- 配对是 Telegram DM 使用的默认令牌交换方式。详情：[配对](/start/pairing)
...
**1. 允许哪些群组**（通过 `channels.telegram.groups` 设置群组白名单）：
...
- Webhook 模式：设置 `channels.telegram.webhookUrl`（可选地设置 `channels.telegram.webhookSecret` + `channels.telegram.webhookPath`）。
  - 本地监听器绑定到 `0.0.0.0:8787`，默认提供 `POST /telegram-webhook`。
  - 如果您的公共 URL 不同，使用反向代理并将 `channels.telegram.webhookUrl` 指向公共端点。

## 回复线程化
Telegram 支持通过标签进行可选的线程化回复：
...
- **动画贴纸（TGS）：** 被跳过（不支持 Lottie 格式进行处理）。
- **视频贴纸（WEBM）：** 被跳过（不支持视频格式进行处理）。
...
贴纸通过 AI 的视觉功能进行处理，以生成描述。由于相同的贴纸经常被反复发送，OpenClaw 会对这些描述进行缓存，以避免重复的 API 调用。
...
代理可以使用 `sticker` 和 `sticker-search` 操作来发送和搜索贴纸。默认情况下这些功能被禁用，必须在配置中启用：
...
参数：
- `fileId`（必需）— 贴纸的 Telegram 文件 ID。在接收贴纸时从 `Sticker.fileId` 获取，或从 `sticker-search` 结果中获取。
- `replyTo`（可选）— 回复的消息 ID。
...
推理流（仅限 Telegram）：
- `/reasoning stream` 将推理流式传输到草稿气泡中，同时生成回复，然后在不含推理的情况下发送最终答案。
- 如果 `channels.telegram.streamMode` 是 `off`，推理流将被禁用。
更多信息：[流式传输 + 分块](/concepts/streaming)。

## 重试策略
出站 Telegram API 调用在出现短暂网络/429 错误时会使用指数退避和抖动进行重试。可通过 `channels.telegram.retry` 进行配置。更多信息请参见 [重试策略](/concepts/retry)。
...
2. 将其转换为具有 `"Telegram reaction added: {emoji} by {user} on msg {id}"` 格式的 **系统事件**
3. 使用与常规消息相同的 **会话密钥** 将系统事件加入队列
4. 当该对话中下一个消息到达时，系统事件会被清空并添加到代理的上下文中
...
- `channels.telegram.reactionLevel`：控制代理的反应能力
  - `"off"` — 代理无法对消息做出反应
  - `"ack"` — 机器人发送确认反应（👀 在处理时）（默认）
  - `"minimal"` — 代理可以谨慎地做出反应（指南：每 5–10 次交流做 1 次）
  - `"extensive"` — 代理可以在适当的情况下自由地做出反应
...
- `openclaw channels status` 显示警告，当配置期望未提及的群组消息时。
- `openclaw channels status --probe` 可额外检查成员资格，以验证显式的数字群组 ID（它无法审核通配符 `"*"` 规则）。
- 快速测试：`/activation always`（仅限会话；使用配置以实现持久性）

**机器人完全看不到群组消息：**
- 如果 `channels.telegram.groups` 被设置，群组必须被列入名单或使用 `"*"`。
- 检查 @BotFather 中的隐私设置 → “群组隐私”应为 **关闭**。
- 确认机器人实际上是成员（而不仅仅是没有读取权限的管理员）。
...
**在 Node 22+ 上，长轮询立即中止（通常与代理/自定义 fetch 一起使用）：**
...
- 一些主机首先将 `api.telegram.org` 解析为 IPv6。如果您的服务器没有正常的 IPv6 出口，gramY 可能会卡在仅 IPv6 的请求上。
...
- `channels.telegram.groups`：每群组的默认值 + 白名单（使用 `"*"` 进行全局默认设置）。
  - `channels.telegram.groups.<id>.requireMention`：提及门控默认。
  - `channels.telegram.groups.<id>.skills`：技能过滤器（omit = 所有技能，空 = 无）。
  ...
  - `channels.telegram.groups.<id>.topics.<threadId>.*`：针对主题的覆盖（与群组相同的字段）。
  - `channels.telegram.groups.<id>.topics.<threadId>.requireMention`：针对主题的提及门控覆盖。
- `channels.telegram.capabilities.inlineButtons`：`off | dm | group | all | allowlist`（默认：白名单）。
- `channels.telegram.accounts.<account>.capabilities.inlineButtons`：针对每个账户的覆盖。
- `channels.telegram.replyToMode`：`off | first | all`（默认：`first`）。
- `channels.telegram.textChunkLimit`：出站分块大小（字符）。
- `channels.telegram.chunkMode`：`length`（默认）或 `newline` 在长度分块前按空行（段落边界）进行分块。
...
- `channels.telegram.reactionNotifications`：`off | own | all` — 控制哪些反应会触发系统事件（默认：`own`，若未设置）。
- `channels.telegram.reactionLevel`：`off | ack | minimal | extensive` — 控制代理的反应能力（默认：`minimal`，若未设置）。
