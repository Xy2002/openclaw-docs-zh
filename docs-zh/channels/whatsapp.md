...
状态：仅通过 Baileys 使用 WhatsApp Web。网关负责维护会话。
...
## 配置写入
默认情况下，WhatsApp 允许根据 `/config set|unset` 触发的配置更新进行写入（需要 `commands.config: true`）。
...
### 专用号码（推荐）
为 OpenClaw 使用**独立的手机号码**。用户体验最佳，路由清晰，无自聊怪癖。理想配置：**备用/旧 Android 手机 + eSIM**。将其保持在 Wi‑Fi 和电源连接下，并通过 QR 码进行绑定。
...
### 个人号码（回退）
快速回退：在**您自己的号码**上运行 OpenClaw。通过给自己发送消息（WhatsApp“给自己发送消息”）进行测试，以免向联系人发送垃圾信息。在设置和实验期间，您可能需要在主手机上查看验证码。**必须启用自聊模式。**

当向导询问您的个人 WhatsApp 号码时，请输入您将用于发送消息的电话号码（所有者/发件人），而不是助手号码。
...
自聊回复默认设置为 `[{identity.name}]`（否则为 `[openclaw]`)，如果未设置 `messages.responsePrefix`。显式设置以自定义或禁用前缀（使用 `""` 删除前缀）。

### 获取号码技巧
- **本地 eSIM**：从您所在国家的移动运营商处获取（最可靠）
  - 奥地利：[hot.at](https://www.hot.at)
  - 英国：[giffgaff](https://www.giffgaff.com) — 免费 SIM 卡，无合约
- **预付费 SIM 卡** — 便宜，只需接收一条短信即可完成验证

**避免：** TextNow、Google Voice 和大多数“免费短信”服务 — WhatsApp 对这些服务采取了严厉的封锁措施。

**提示：** 号码只需接收一条验证短信。之后，WhatsApp Web 会话可通过 `creds.json` 持续存在。

## 为什么不用 Twilio？
- 早期的 OpenClaw 版本支持 Twilio 的 WhatsApp Business 集成。
- WhatsApp Business 号码并不适合用作个人助理。
- Meta 强制执行 24 小时回复窗口；如果您在过去 24 小时内没有回复，该商业号码将无法发起新消息。
- 高流量或“频繁聊天”的使用会触发严格的封锁，因为商业账户并不适用于发送大量个人助理消息。
- 结果：交付不可靠且经常被封锁，因此已取消对该集成的支持。

## 登录 + 凭证
- 登录命令：`openclaw channels login`（通过已链接设备扫描 QR 码）。
- 多账号登录：`openclaw channels login --account <id>`（`<id>` = `accountId`）。
- 默认账号（当 `--account` 未指定时）：如果有 `default`，则使用该账号；否则使用第一个已配置账号 ID（按排序）。
- 凭证存储在 `~/.openclaw/credentials/whatsapp/<accountId>/creds.json` 中。
- 备份副本存储在 `creds.json.bak` 中（在损坏时恢复）。
...
- 注销后的套接字会导致错误，并提示重新链接。

## 入站流程（私信 + 群组）
- WhatsApp 事件来自 `messages.upsert`（Baileys）。
- 收件箱监听器在关闭时会被分离，以避免在测试/重启中累积事件处理器。
- 状态/广播聊天会被忽略。
...
- **私信策略**：`channels.whatsapp.dmPolicy` 控制直接聊天的访问权限（默认： `pairing`）。
  ...
  - 您已链接的 WhatsApp 号码被视为受信任，因此自消息会跳过 `channels.whatsapp.dmPolicy` 和 `channels.whatsapp.allowFrom` 检查。

### 个人号码模式（回退）
如果您在**个人 WhatsApp 号码**上运行 OpenClaw，则启用 `channels.whatsapp.selfChatMode`（见上文示例）。

行为：
- 出站私信永远不会触发配对回复（防止向联系人发送垃圾信息）。
- 入站未知发件人仍遵循 `channels.whatsapp.dmPolicy`。
- 自聊模式（allowFrom 包含您的号码）可避免自动读取回执，并忽略提及 JID。
- 非自聊私信会发送读取回执。
...
不会。默认的私信策略是**配对**，因此未知发件人只会收到配对代码，而他们的消息**不会被处理**。OpenClaw 只会回复它收到的聊天消息，或者您明确触发的发送（代理/CLI）。

**WhatsApp 上的配对是如何工作的？**  
配对是针对未知发件人的私信门控：
- 来自新发件人的第一条私信会返回一个短代码（消息不会被处理）。
- 批准方式：`openclaw pairing approve whatsapp <code>`（通过 `openclaw pairing list whatsapp` 列表批准）。
- 代码 1 小时后失效；每个频道的待处理请求上限为 3 个。

**是否有多个人可以使用不同的 OpenClaw 实例在一个 WhatsApp 号码上？**  
是的，可以通过 `bindings` 将每个发件人路由到不同的代理（peer `kind: "dm"`，发件人 E.164 如 `+15551234567`)。回复仍然来自**同一个 WhatsApp 账号**，直接聊天会合并到每个代理的主要会话中，因此请使用**每人一个代理**。私信访问控制（`dmPolicy`/`allowFrom`）是针对 WhatsApp 账号的全局设置。详情参见 [多代理路由](/concepts/multi-agent)。

**为什么向导会要求我的手机号码？**  
向导使用它来设置您的**白名单/所有者**，以便允许您自己的私信。它不会用于自动发送。如果您在个人 WhatsApp 号码上运行，请使用该号码并启用 `channels.whatsapp.selfChatMode`。
...
- 所有者 = `channels.whatsapp.allowFrom`（或未设置时为自我 E.164）。
- **历史注入**（仅限待处理消息）：
  - 最近 *未处理* 的消息（默认 50 条）插入到：
...
## 回复传递（线程化）
- 在当前网关中，WhatsApp Web 发送标准消息（不支持引用回复线程化）。
- 在此渠道中，回复标签会被忽略。
...
- 可选：`participant`（群组发件人）、`fromMe`（对自己消息作出反应）、`accountId`（多账号）。
- 反应移除语义：参见 [/tools/reactions](/tools/reactions)。
- 工具门控：`channels.whatsapp.actions.reactions`（默认：启用）。

## 限制
- 出站文本分块至 `channels.whatsapp.textChunkLimit`（默认 4000）。
...
- 文本分块：每条消息最多 4k（可通过 `channels.whatsapp.textChunkLimit` 进行配置，可选 `channels.whatsapp.chunkMode`）。
- 媒体：
  - 支持图片/视频/音频/文档。
  - 音频以 PTT 格式发送；`audio/ogg` => `audio/ogg; codecs=opus`。
  - 仅在第一件媒体上添加说明。
  - 媒体获取支持 HTTP(S) 和本地路径。
...
- `channels.whatsapp.ackReaction`（消息收到时自动反应：`{emoji, direct, group}`）。
...
- `channels.whatsapp.dmHistoryLimit`（用户回合中的私信历史限制）。按用户覆盖：`channels.whatsapp.dms["<phone>"].historyLimit`。
- `channels.whatsapp.groups`（群组白名单 + 提及门控默认；使用 `"*"` 允许所有人）。
...
**Bun 运行时**
- 不建议使用 Bun。WhatsApp（Baileys）和 Telegram 在 Bun 上表现不稳定。
  请使用 **Node** 运行网关。（参见入门运行时说明。）
