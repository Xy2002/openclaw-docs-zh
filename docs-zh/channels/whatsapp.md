...
- 确定性路由：回复返回到 WhatsApp，不经过模型路由。
- 模型能够获取足够的上下文来理解引用回复。

## 配置写入
默认情况下，WhatsApp 允许在 `/config set|unset` 触发时更新配置（需要 `commands.config: true`）。
...
### 专用号码（推荐）
为 OpenClaw 使用**独立的电话号码**。用户体验最佳，路由清晰，无自聊怪癖。理想设置：**备用/旧 Android 手机 + eSIM**。将其保持在 Wi‑Fi 和电源连接下，并通过 QR 码进行绑定。
...
**配对模式（可选）：**
如果您希望使用配对而不是白名单，请将 `channels.whatsapp.dmPolicy` 设置为 `pairing`。未知发件人会收到配对代码；批准方式：
...
自聊回复默认设置为 `[{identity.name}]`（否则为 `[openclaw]`)，如果 `messages.responsePrefix` 未设置。显式设置以自定义或禁用前缀（使用 `""` 可移除前缀）。
...
**避免：** TextNow、Google Voice 和大多数“免费 SMS”服务——WhatsApp 对这些服务采取了极为严格的封禁措施。
...
- Meta 强制执行 24 小时回复窗口；如果在过去 24 小时内没有回复，该商业号码将无法发起新消息。
- 高流量或“频繁聊天”的使用会触发严厉的封禁，因为商业账户并非设计用于发送大量个人助理消息。
- 结果：交付不可靠且经常被封禁，因此已取消对该功能的支持。

## 登录 + 凭据
- 登录命令：`openclaw channels login`（通过已链接设备扫描 QR 码）。
- 多账户登录：`openclaw channels login --account <id>`（`<id>` = `accountId`）。
- 默认账户（当 `--account` 未指定时）：如果有 `default`，则使用该账户；否则使用第一个已配置账户 ID（按排序）。
...
- 收件箱监听器在关闭时会被分离，以避免在测试/重启中累积事件处理程序。
- 状态/广播聊天会被忽略。
...
- **私信策略**：`channels.whatsapp.dmPolicy` 控制直接聊天的访问权限（默认：`pairing`）。
  ...
  - 您已链接的 WhatsApp 号码被视为隐式信任，因此自消息会跳过 `channels.whatsapp.dmPolicy` 和 `channels.whatsapp.allowFrom` 检查。
...
- 自聊模式（allowFrom 包含您的号码）会避免自动读取回执，并忽略提及 JID。
- 非自聊私信会发送读取回执。
...
**当我绑定 WhatsApp 时，OpenClaw 会随机向联系人发送消息吗？**  
不会。默认的私信策略是**配对**，因此未知发件人只会收到配对代码，他们的消息**不会被处理**。OpenClaw 只会回复它收到的聊天消息，或者您明确触发的发送（代理/CLI）。

**WhatsApp 上的配对是如何工作的？**  
配对是针对未知发件人的私信门控：
- 新发件人发出的第一条私信会返回一个短代码（消息不会被处理）。
- 批准方式：`openclaw pairing approve whatsapp <code>`（通过 `openclaw pairing list whatsapp` 列表批准）。
- 代码 1 小时后失效；每个通道待处理的请求上限为 3 个。

**是否有多个人可以使用不同的 OpenClaw 实例在一个 WhatsApp 号码上？**  
是的，可以通过 `bindings` 将每个发件人路由到不同的代理（peer `kind: "dm"`，发件人 E.164 如 `+15551234567`)。回复仍然来自**同一个 WhatsApp 账号**，直接聊天会合并到每个代理的主要会话中，因此请为每个人使用**一个代理**。私信访问控制（`dmPolicy`/`allowFrom`）对每个 WhatsApp 账号是全局的。详情请参阅 [多代理路由](/concepts/multi-agent)。

**为什么向导会要求我的电话号码？**  
向导使用它来设置您的**白名单/所有者**，以便允许您自己的私信。它不会用于自动发送。如果您在个人 WhatsApp 号码上运行，请使用相同的号码并启用 `channels.whatsapp.selfChatMode`。
...
- 激活模式：
  ...
  - `always`：始终触发。
- `/activation mention|always` 仅供所有者使用，必须作为单独消息发送。
- 所有者 = `channels.whatsapp.allowFrom`（或未设置时为自 E.164）。
- **历史注入**（仅待处理消息）：
  - 最近 *未处理* 的消息（默认 50 条）插入到以下内容中：
...
- 媒体：
  ...
  - 仅在第一条媒体项目上添加标题。
  - 媒体获取支持 HTTP(S) 和本地路径。
  - 动画 GIF：WhatsApp 期望使用 MP4 格式，并配合 `gifPlayback: true` 实现内联循环。
...
- 解决方法：在网关主机上运行 `openclaw channels login`，然后扫描 QR 码（WhatsApp → 设置 → 已链接设备）。
