...
## 目标
- 通过 Discord 私信或服务器频道与 OpenClaw 通信。
...
4. 运行网关；当令牌可用时（优先使用配置，环境变量作为后备）且 `channels.discord.enabled` 不等于 `false` 时，它会自动启动 Discord 频道。
   - 如果您更喜欢环境变量，请设置 `DISCORD_BOT_TOKEN`（可选配置块）。
5. 直接聊天：交付时使用 `user:<id>`（或 `<@id>` 提及）；所有回合都会进入共享的 `main` 会话。纯数字 ID 模糊不清，会被拒绝。
6. 服务器频道：使用 `channel:<channelId>` 进行交付。默认需要提及，可以按服务器或按频道设置。
7. 直接聊天：默认通过 `channels.discord.dm.policy` 保护（默认：`"pairing"`）。未知发件人会收到配对代码（1小时后失效）；通过 `openclaw pairing approve discord <code>` 批准。
...
9. 可选的服务器规则：按服务器 ID（首选）或别名设置 `channels.discord.guilds`，并提供按频道的规则。
...
注意：名称 → ID 解析使用服务器成员搜索，需要服务器成员意图；如果机器人无法搜索成员，使用 IDs 或 `<@id>` 提及。
注意：别名小写，空格替换为 `-`。频道名称被转换为别名，不带前导 `#`。
注意：Guild 上下文 `[from:]` 行包括 `author.tag` + `id`，使 ping 准备就绪的回复变得容易。
...
在 **机器人** → **特权网关意图** 中，启用：
- **消息内容意图**（在大多数 guild 中读取消息文本所必需；如果没有它，您会看到“使用了不允许的意图”或机器人会连接但不会对消息作出反应）
- **服务器成员意图**（推荐；某些成员/用户查找和 guild 白名单匹配所必需）
...
Discord 到处使用数字 ID；OpenClaw 配置更倾向于使用 ID。
...
- `agents.list[].groupChat.mentionPatterns`（或 `messages.groupChat.mentionPatterns`）也视为 guild 消息的提及。
...
- “私信不起作用”：`channels.discord.dm.enabled=false`、`channels.discord.dm.policy="disabled"`，或者您尚未获得批准（`channels.discord.dm.policy="pairing"`）。

## 功能与限制
- 私信和 guild 文本频道（线程被视为单独的频道；不支持语音）。
- 输入指示器尽力而为；消息分块使用 `channels.discord.textChunkLimit`（默认 2000）并根据行数拆分长回复（`channels.discord.maxLinesPerMessage`，默认 17）。
...
## 重试策略
出站 Discord API 调用在遇到速率限制（429）时，如果可用，将使用 Discord 的 `retry_after` 进行重试，并采用指数退避和抖动。可通过 `channels.discord.retry` 配置。参见 [重试策略](/concepts/retry)。
...
- `dm.allowFrom`: DM 白名单（用户 ID 或姓名）。被 `dm.policy="allowlist"` 使用，并用于 `dm.policy="open"` 验证。向导接受用户名，并在机器人可以搜索成员时将其解析为 ID。
- `dm.groupEnabled`: 启用群组 DM（默认 `false`）。
- `dm.groupChannels`: 可选的群组 DM 频道 ID 或别名白名单。
...
- `guilds.<id>.users`: 可选的 per-guild 用户白名单（ID 或姓名）。
...
- `guilds.<id>.channels.<channel>.allow`: 当 `groupPolicy="allowlist"` 时，允许或拒绝该频道。
- `guilds.<id>.channels.<channel>.requireMention`: 对该频道进行提及控制。
...
- `guilds.<id>.channels.<channel>.skills`: 技能过滤器（omit = 所有技能，empty = 无技能）。
- `guilds.<id>.channels.<channel>.systemPrompt`: 频道的额外系统提示（与频道主题结合）。
- `guilds.<id>.channels.<channel>.enabled`: 设置 `false` 以禁用该频道。
...
- `chunkMode`: `length`（默认）仅在超过 `textChunkLimit` 时才会拆分；`newline` 在长度分块之前按空行（段落边界）拆分。
- `maxLinesPerMessage`: 每条消息的软最大行数。默认：17。
- `mediaMaxMb`: 对保存到磁盘的传入媒体进行限制。
- `historyLimit`: 在回复提及时纳入上下文的最近 guild 消息数量（默认 20；回退到 `messages.groupChat.historyLimit`；`0` 禁用）。
...
| polls | 开启 | 创建投票 |
| permissions | 开启 | 频道权限快照 |
| messages | 开启 | 读取/发送/编辑/删除 |
| threads | 开启 | 创建/列举/回复 |
| pins | 开启 | 固定/解除固定/列举 |
| search | 开启 | 消息搜索（预览功能） |
| memberInfo | 开启 | 成员信息 |
...
白名单匹配注意事项：
- `allowFrom`/`users`/`groupChannels` 接受 IDs、名称、标签或提及，如 `<@id>`。
...
- 读取/搜索/固定工具的有效载荷包括归一化的 `timestampMs`（UTC 精确时间毫秒）和 `timestampUtc` 与原始 Discord `timestamp` 一起。
...
Discord 消息 IDs 在注入的上下文中呈现（`[discord message id: …]` 和历史行），以便代理可以针对它们。
...
- 如果机器人卡住或受到速率限制，重启网关（`openclaw gateway --force`）后，确认没有其他进程占用 Discord 会话。
