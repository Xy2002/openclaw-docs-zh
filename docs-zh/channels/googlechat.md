...
5) 在 [Google Cloud 控制台的 Chat 配置](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat)中创建 Google Chat 应用：
   ...
   - 在 **触发器** 下，选择 **对所有触发器使用通用 HTTP 终端点 URL**，并将其设置为您的网关公共 URL 后加上 `/googlechat`。
...
## 添加到 Google Chat
当网关运行且您的电子邮件已添加到可见性列表时：
1) 访问 [Google Chat](https://chat.google.com/)。
2) 点击 **直接消息** 旁边的 **+**（加号）图标。
3) 在搜索栏中（您通常用来添加联系人的地方），输入您在 Google Cloud 控制台中配置的 **应用名称**。
   - **注意**：由于该机器人是私有应用，因此不会出现在“市场”浏览列表中。您必须按名称搜索它。
4) 从搜索结果中选择您的机器人。
5) 点击 **添加** 或 **聊天** 以开始一对一对话。
6) 发送“你好”以触发助手！

## 公共 URL（仅限 Webhook）
Google Chat Webhook 需要一个公共 HTTPS 终端点。出于安全考虑，**仅将 `/googlechat` 路径暴露给互联网**。请将 OpenClaw 仪表板和其他敏感端点保留在您的私有网络中。

### 选项 A：Tailscale Funnel（推荐）
使用 Tailscale Serve 处理私有仪表板，使用 Funnel 处理公共 Webhook 路径。这样可以保持 `/` 私密，同时仅公开 `/googlechat`。
...
4. **授权节点以进行 Funnel 访问：**
   如果出现提示，请访问输出中显示的授权 URL，以在您的尾网策略中为此节点启用 Funnel。
...
```
使用此配置，任何对 `your-domain.com/` 的请求都会被忽略或返回 404，而 `your-domain.com/googlechat` 则会安全地路由到 OpenClaw。

### 选项 C：Cloudflare Tunnel
配置您的隧道入口规则，使其仅路由 Webhook 路径：
...
- 可通过 `reactions` 工具和 `channels action` 使用反应功能，前提是已启用 `actions.reactions`。
- `typingIndicator` 支持 `none`、`message`（默认）和 `reaction`（反应需要用户 OAuth）。
- 附件通过 Chat API 下载，并存储在媒体管道中（大小上限由 `mediaMaxMb` 规定）。
...
- 在发送测试消息时，使用 `openclaw logs --follow` 以查看请求是否到达网关。
