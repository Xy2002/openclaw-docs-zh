...
- 仅包含指令的消息可切换会话的详细模式，并回复 `Verbose logging enabled.` / `Verbose logging disabled.`；无效层级将返回提示而不改变状态。
...
- 默认情况下，心跳仅发送最终有效载荷。如需同时发送单独的 `Reasoning:` 消息（若可用），请设置 `agents.defaults.heartbeat.includeReasoning: true` 或针对单个代理的 `agents.list[].heartbeat.includeReasoning: true`。
