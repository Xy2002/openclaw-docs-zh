...
- 该设置在当前会话中生效（默认按发件人区分）；可通过 `/think:off` 或会话空闲重置清除。
- 系统会发送确认回复（`Thinking level set to high.` / `Thinking disabled.`）。如果层级无效（如 `/thinking big`），命令将被拒绝并给出提示，会话状态保持不变。
...
- 内联指令仅影响该消息；其他情况下适用会话或全局默认值。
- 发送 `/verbose`（或 `/verbose:`）且不带参数，即可查看当前的详细模式层级。
- 当详细模式开启时，发出结构化工具结果的代理（Pi、其他 JSON 代理）会将每次工具调用作为单独的元数据消息返回，若可用则以 `<emoji> <tool-name>: <arg>` 为前缀（路径/命令）。这些工具摘要会在每个工具开始时立即发送（独立气泡），而非以流式增量形式发送。
...
- 选择其他层级仅适用于下一条消息（`thinkingOnce`）；发送后，选择器会自动恢复为已保存的会话层级。
- 若要更改会话默认值，请发送一条 `/think:<level>` 指令（同之前）；选择器将在下次重新加载后反映这一更改。
