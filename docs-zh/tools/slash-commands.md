...
此外，还有一些**内联快捷方式**（仅限白名单/授权发件人）：`/help`、`/commands`、`/status`、`/whoami`（`/id`）。
...
- `commands.native`（默认 `"auto"`）注册原生命令。
  - 自动：Discord/Telegram 上启用；Slack 上禁用（直到你添加斜杠命令）；对于不支持原生命令的提供商，则忽略。
  ...
  - `false` 在启动时清除 Discord/Telegram 上先前注册的命令。Slack 命令由 Slack 应用管理，不会自动删除。
...
- `/verbose` 用于调试和额外的可见性；在正常使用中请将其**关闭**。
- `/reasoning`（和 `/verbose`）在群组环境中存在风险：它们可能会泄露你不打算公开的内部推理或工具输出。尤其是在群聊中，最好将其关闭。
- **快速通道**：来自白名单发件人的仅命令消息会立即处理（绕过队列 + 模型）。
- **群组提及限制**：来自白名单发件人的仅命令消息无需满足提及要求。
- **内联快捷方式（仅限白名单发件人）**：某些命令也可以嵌入到普通消息中，并在模型看到剩余文本之前被剥离。
  - 例如：`hey /status` 触发状态回复，剩余文本则按正常流程继续处理。
- 目前：`/help`、`/commands`、`/status`、`/whoami`（`/id`）。
- 未授权的仅命令消息会被静默忽略，内联的 `/...` 标记会被视为纯文本。
- **技能命令**：`user-invocable` 技能以斜杠命令的形式公开。名称会被清理为 `a-z0-9_`（最多 32 个字符）；冲突的名称会附加数字后缀（例如 `_2`）。
  - `/skill <name> [input]` 按名称运行技能（当原生命令限制阻止每个技能单独命令时很有用）。
  - 默认情况下，技能命令会作为普通请求转发给模型。
  - 技能可以选择声明 `command-dispatch: tool`，以将命令直接路由到工具（确定性，无需模型）。
  - 例如：`/prose`（OpenProse 插件）——参见 [OpenProse](/prose)。
- **原生命令参数**：Discord 对于动态选项使用自动完成（当你省略必填参数时，则使用按钮菜单）。Telegram 和 Slack 在命令支持选项且你省略参数时会显示按钮菜单。
...
`/debug` 允许你设置**仅运行时**的配置覆盖（内存，而非磁盘）。仅限所有者。默认关闭；通过 `commands.debug: true` 启用。
...
`/config` 写入你的磁盘配置（`openclaw.json`）。仅限所有者。默认关闭；通过 `commands.config: true` 启用。
...
- **Slack**：`channels.slack.slashCommand` 仍支持单个 `/openclaw` 式命令。如果你启用 `commands.native`，则必须为每个内置命令创建一个 Slack 斜杠命令（与 `/help` 名称相同）。Slack 的命令参数菜单以短暂的 Block Kit 按钮形式呈现。
