...
- 许多断言还配有对应的**反例模型**，用于针对某一类现实中的漏洞生成反例轨迹。

**尚不包括的内容：** 目前尚未提供“OpenClaw 在所有方面都是安全的”这一证明，也未验证完整的 TypeScript 实现是否正确。
...
- 检查结果受限于 TLC 探索的状态空间；即使结果显示为“绿色”，也不意味着在超出建模假设和边界的情况下仍然安全。
- 部分断言依赖于明确的环境假设（例如，正确的部署方式、正确的配置输入）。

## 结果重现

目前，可通过在本地克隆模型仓库并运行 TLC 来重现结果（见下文）。未来的迭代可能提供：
...
**断言：** `nodes.run` 要求 (a) 节点命令白名单加上已声明的命令，以及 (b) 在配置时进行实时批准；在模型中，批准通过令牌化来防止重放攻击。
...
**断言：** 不同对等方之间的 DM 不会在未显式关联或配置的情况下合并到同一个会话中。
...
**断言：** 配对存储应确保 `MaxPending` 和幂等性，即使在交错执行的情况下也是如此（即，“先检查后写入”必须是原子操作或加锁操作；刷新不应产生重复条目）。
...
**断言：** 在扇出过程中，摄取过程应保持轨迹相关性，并且在提供商重试时具有幂等性。
...
**断言：** 路由默认应保持 DM 会话隔离，仅在显式配置时才会合并会话（通道优先级 + identityLinks）。

含义：
- 特定通道的 dmScope 优先级设置必须覆盖全局默认设置。
- identityLinks 只应在显式关联的群组内生效，而不应在无关的对等方之间生效。
