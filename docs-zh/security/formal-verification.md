...
**断言：** 如果在没有身份验证的情况下绑定到环回以外的地址，可能会导致远程入侵的可能性增加；根据模型假设，令牌或密码可以阻止未认证的攻击者。
...
**断言：** `nodes.run` 要求：(a) 节点命令白名单加上已声明的命令；(b) 在启用时需实时批准；批准过程采用令牌机制以防止重放攻击（在模型中体现）。
...
**断言：** 配对存储应在交错操作下仍能强制执行 `MaxPending` 并保持幂等性（即，“先检查后写入”必须是原子操作或加锁操作；刷新不应产生重复条目）。

含义：
- 在并发请求下，某个通道的 `MaxPending` 上限不能被突破。
...
- 重试不会导致重复处理。
- 如果缺少提供商事件 ID，去重机制将回退到一个安全键（如轨迹 ID），以避免遗漏不同的事件。
