...
## 快速检查
- `openclaw status` — 本地摘要：网关可达性/模式、更新提示、关联通道认证时效、会话数及近期活动。
...
- 日志：使用 tail `/tmp/openclaw/openclaw-*.log` 并过滤出 `web-heartbeat`、`web-reconnect`、`web-auto-reply`、`web-inbound`。
...
## 专用“健康”命令
`openclaw health --json` 向正在运行的网关请求其健康快照（命令行界面不直接使用通道套接字）。在可用时，它会报告关联凭据和认证时效、各通道的探测摘要、会话存储摘要以及探测持续时间。如果网关无法访问或探测失败/超时，该命令将以非零退出码退出。使用 `--timeout <ms>` 可覆盖默认的 10 秒超时时间。
