...
当您的笔记本电脑经常休眠但您希望代理始终在线时，这是理想的选择。

### 2) 家用桌面运行网关，笔记本电脑作为远程控制端
...
- `openclaw gateway {status,health,send,agent,call}` 也可以在需要时通过 `--url` 指向转发后的 URL。
...
macOS 菜单栏应用程序可以端到端地完成相同设置（远程状态检查、WebChat 和语音唤醒转发）。
...
- **非环回绑定**（`lan`/`tailnet`/`custom`，或在无法使用环回时使用 `auto`）必须使用身份验证令牌或密码。
