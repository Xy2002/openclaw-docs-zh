...
- **网关**：一个长期运行的网关进程，负责维护状态（会话、配对、节点注册表）并运行通道。大多数设置为每台主机使用一个网关；也可以采用隔离的多网关设置。
- **网关 WS（控制平面）**：默认情况下位于 `127.0.0.1:18789` 上的 WebSocket 端点；可通过 `gateway.bind` 绑定到局域网或尾网。
- **直接 WS 传输**：面向局域网/尾网的网关 WS 端点（无需 SSH）。
...
- **macOS 应用程序**：帮助您选择网关，显示配对提示，并仅将 SSH 用作回退方案。
