...
- **网关**：一个长期运行的网关进程，负责管理状态（会话、配对、节点注册表）并运行通道。大多数部署方案为每台主机配备一个网关；也可采用隔离的多网关架构。
- **网关 WS（控制平面）**：默认情况下位于 `127.0.0.1:18789` 上的 WebSocket 端点；可通过 `gateway.bind` 绑定到局域网或尾网。
- **直接 WS 传输**：面向局域网或尾网的网关 WS 端点（无需 SSH）。
- **SSH 传输（回退）**：通过在 SSH 上转发 `127.0.0.1:18789` 实现远程控制。
...
- **直接 WS** 在同一网络和尾网内提供最佳用户体验：
  - 通过 Bonjour 在局域网中实现自动发现
  - 配对令牌和 ACL 由网关管理
  - 无需 shell 访问；协议表面简洁且易于审计
- **SSH** 仍然是通用的回退方案：
  - 只要具备 SSH 访问权限即可使用（即使跨不相关网络）
  - 能够应对组播/mDNS 问题
  - 除了 SSH 外无需开放新的入站端口
...
当不存在直接路由（或直接路由被禁用）时，客户端始终可以通过 SSH 连接到环回网关端口来建立连接。
...
1) 如果已配置并可到达配对的直接端点，则使用该端点。
2) 否则，如果 Bonjour 在局域网中找到网关，则提供一键“使用此网关”的选项，并将其保存为直接端点。
3) 否则，如果已配置尾网 DNS/IP，则尝试直接连接。
4) 否则，回退到 SSH。
...
- **网关**：发布发现信标，负责配对决策，并托管 WS 端点。
- **macOS 应用程序**：帮助您选择网关，显示配对提示，并仅将 SSH 作为回退方案。
- **iOS/Android 节点**：作为便利功能浏览 Bonjour，并连接到已配对的网关 WS。
