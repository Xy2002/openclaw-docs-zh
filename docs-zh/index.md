...
大多数操作都通过 **Gateway**（`openclaw gateway`）进行，这是一个长期运行的单一进程，负责管理通道连接和 WebSocket 控制平面。
...
- **Canvas 主机：** HTTP 文件服务器位于 `canvasHost.port`（默认 `18793`），为节点 WebView 提供 `/__openclaw__/canvas/`；请参阅 [网关配置](/gateway/configuration) (`canvasHost`)。
