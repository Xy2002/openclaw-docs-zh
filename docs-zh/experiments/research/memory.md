...
目标：打造一种类似 Clawd 的工作区（`agents.defaults.workspace`，默认 `~/.openclaw/workspace`），其中“记忆”以每日一个 Markdown 文件的形式存储（`memory/YYYY-MM-DD.md`），外加一小部分稳定文件（如 `memory.md`、`SOUL.md`）。
...
但在以下方面存在不足：
- 高召回率检索（“我们对 X 做过什么决定？”、“我们上次尝试 Y 是什么时候？”）
...
- **可解释性**：检索到的条目应可追溯来源（文件 + 位置），并与推理过程明确分离。
- **低仪式感**：日常记录仍采用 Markdown 格式，无需复杂的模式设计。
- **渐进式演进**：v1 版本仅使用 FTS 即可实用；语义/向量和图谱功能可作为可选升级。
- **代理友好**：便于在标记预算内进行“回忆”，返回小批量的事实片段。

## 北斗星模型（Hindsight × Letta）
需要融合两部分内容：

1) **Letta/MemGPT 式控制循环**
- 保持一个小规模的“核心”始终处于上下文中（角色 + 关键用户事实）
- 其余内容则处于上下文之外，通过工具检索
- 记忆写入是显式的工具调用（追加/替换/插入），持久化后在下一轮中重新注入
...
## 保留 / 回忆 / 反思（操作循环）
### 保留：将每日日志归一化为“事实”
Hindsight 的关键洞见在此处至关重要：存储 **叙事性的、自包含的事实**，而非细碎的片段。

针对 `memory/YYYY-MM-DD.md` 的实用规则：
- 在一天结束时（或过程中），添加一个 `## Retain` 小节，包含 2–5 条要点，要求：
  - 具备叙事性（跨轮次上下文得以保留）
  - 自包含（单独存在时也具有意义）
  - 标记类型及提及的实体
...
最低限度的解析：
- 类型前缀：`W`（世界）、`B`（经验/传记）、`O`（意见）、`S`（观察/摘要；通常由系统生成）
- 实体：`@Peter`、`@warelay` 等（别名映射到 `bank/entities/*.md`）
- 意见置信度：`O(c=0.0..1.0)` 为可选项

若不希望作者费心思考，反思作业可以从日志的其余部分推断出这些要点，但显式设置 `## Retain` 小节是最简单的“质量杠杆”。
...
- `["Peter","warelay"]`（叙事性事实）
- `content`（相关证据 ID）
- `source`（其他相关信息，如 `memory/2025-11-27.md#L12` 等）

### 反思：生成稳定页面并更新信念
反思是一项定时任务（每日或心跳触发的 `ultrathink`），其作用包括：
...
- 每个意见包含：
  ...
  - 证据链接（支持性和矛盾性事实的 ID）
- 当新事实到来时：
  - 通过实体重叠和相似性找到候选意见（先用 FTS，后续再用嵌入）
  - 置信度以小幅度调整；大幅变动需有强有力的矛盾证据和重复证据的支持
...
如果“S-Collide”指的是 **SuCo（子空间碰撞）**：这是一种 ANN 检索方法，通过在子空间中使用学习过的/结构化的碰撞来实现强召回与延迟之间的权衡（论文：arXiv 2411.14754，2024）。

针对 `~/.openclaw/workspace` 的务实建议：
- 不要一开始就使用 SuCo。
- 先使用 SQLite FTS +（可选）简单的嵌入；这样你很快就能获得大部分用户体验上的收益。
...
开放问题：
- 在你的设备上（笔记本电脑 + 台式机），哪种 **最佳** 离线嵌入模型适用于“个人助理记忆”？
  - 如果你已经使用 Ollama：使用本地模型进行嵌入；否则，在工具链中搭载一个小型嵌入模型。

## 最小可用试点
如果你想要一个最小但仍然有用的版本：

- 在每日日志中添加 `bank/` 实体页面和一个 `## Retain` 小节。
- 使用 SQLite FTS 进行带引用的回忆（路径 + 行号）。
- 仅在召回质量和规模有更高要求时才添加嵌入。

## 参考文献
- Letta / MemGPT 概念：核心记忆块 + 存档记忆 + 工具驱动的自我编辑记忆。
- Hindsight 技术报告：保留 / 回忆 / 反思、四网络记忆、叙事性事实提取、意见置信度演变。
