...
1) **选择计划**
   ...
   - 定期作业 → `schedule.kind = "every"` 或 `schedule.kind = "cron"`
...
#### 孤立作业（专用 cron 会话）
孤立作业在 `cron:<jobId>` 会话中运行一个专用代理回合。
...
- 如果 `payload.deliver: true`，则将输出传递到频道；否则保持内部。
...
- `agentTurn`：仅限孤立会话，运行一个专用代理回合。

常见的 `agentTurn` 字段：
- `message`：必需的文本提示。
- `model` / `thinking`：可选覆盖（见下文）。
- `timeoutSeconds`：可选超时覆盖。
- `deliver`：`true` 可将输出发送到频道目标。
- `channel`：`last` 或特定频道。
...
如果 `channel` 或 `to` 被省略，cron 可以回退到主会话的“最后路由”（代理上次回复的地方）。

交付注意事项：
- 如果设置了 `to`，cron 会自动传递代理的最终输出，即使 `deliver` 被省略。
