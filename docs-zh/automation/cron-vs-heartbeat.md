...
| 每天早上 9 点准时发送日报 | Cron（独立） | 需要精确的时间安排 |
| 监控日历以获取即将发生的事件 | 心跳 | 非常适合周期性状态感知 |
| 每周运行深度分析 | Cron（独立） | 独立任务，可使用不同模型 |
...
- **上下文感知决策**：代理拥有完整的主会话上下文，能够智能判断哪些任务紧急，哪些可以稍后处理。
- **对话连续性**：心跳运行共享同一会话，因此代理能记住最近的对话并自然地跟进。
- **低开销监控**：一个心跳即可替代多个小型轮询任务。

### 心跳的优势

- **批量处理多项检查**：一次代理运行即可同时检查收件箱、日历和通知。
- **减少 API 调用**：单个心跳的成本低于 5 个独立的 Cron 作业。
- **上下文感知**：代理了解你当前的工作内容，并据此优先处理相关任务。
- **智能抑制**：如果没有需要关注的内容，代理会回复 `HEARTBEAT_OK`，不会发送任何消息。
- **自然的时间安排**：根据队列负载略有偏差，但这对大多数监控任务来说是可接受的。
...
Cron 作业在**精确的时间点**运行，并且可以在独立会话中执行，而不影响主会话上下文。

### 何时使用 Cron

- **需要精确时间**：“每周一上午 9:00 发送此消息”（而不是“大约 9 点”）。
- **独立任务**：不需要对话上下文的任务。
...
- **交付控制**：可以直接发送到某个渠道；默认情况下仍会在主会话中发布摘要（可配置）。
...
当任务超出单次代理运行的范围，并且你需要一个可恢复的工作流及人工检查点时，就该使用 Lobster。

### Lobster 的适用场景

- **多步骤自动化**：你需要固定的工具调用流程，而不仅仅是一次性提示。
...
- **心跳/Cron**决定**何时**运行。
- **Lobster**定义**运行开始后执行哪些步骤**。

对于计划的工作流，使用 Cron 或心跳触发代理运行，进而调用 Lobster。
...
| | 心跳 | Cron（主会话） | Cron（独立会话） |
|---|---|---|---|
| 会话 | 主会话 | 主会话（通过系统事件） | `cron:<jobId>` |
| 历史 | 共享 | 共享 | 每次运行都是全新历史 |
| 上下文 | 完整 | 完整 | 无（从干净状态开始） |
| 模型 | 主会话模型 | 主会话模型 | 可覆盖 |
| 输出 | 若未返回 `HEARTBEAT_OK`，则会交付 | 心跳提示 + 事件 | 摘要发布到主会话 |
...
| 机制 | 成本特点 |
|-----------|--------------|
| 心跳 | 每 N 分钟一次运行；成本随 HEARTBEAT.md 大小变化 |
| Cron（主会话） | 在下次心跳时添加事件（无需独立运行） |
| Cron（独立会话） | 每个作业都需要一次完整的代理运行；可使用更便宜的模型 |

**提示**：
- 保持 `HEARTBEAT.md` 简短，以最小化 token 开销。
- 将相似的检查合并到心跳中，而不是使用多个 Cron 作业。
- 如果只需要内部处理，可在心跳中使用 `target: "none"`。
- 对于常规任务，可使用廉价模型的独立 Cron。
