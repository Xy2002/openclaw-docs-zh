...
## 摘要
通过 Node 运行 OpenClaw 时，使用 `tsx` 会在启动时失败，错误信息如下：
...
## 备注/假设
- `tsx` 使用 esbuild 将 TS/ESM 转换为其他格式。esbuild 的 `keepNames` 会发出一个 `__name` 帮助函数，并用 `__name(...)` 包装函数定义。
...
- 使用 `tsx` 测试 Node LTS 版本（22/24），以确定问题是否仅限于 Node 25。
...
- 如果已知回归问题，可尝试使用 nightly 版本的 `tsx`，或锁定到较早版本进行测试。
- 如果问题在 Node LTS 上也能复现，请向上游提交包含 `__name` 堆栈跟踪的最小复现示例。
