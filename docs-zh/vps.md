...
- 默认安全策略：将网关保留在环回地址上，并通过 SSH 隧道或 Tailscale Serve 访问它。
  如果您绑定到 `lan`/`tailnet`，则需要启用 `gateway.auth.token` 或 `gateway.auth.password`。
...
（Mac/iOS/Android/无头设备）。节点提供本地屏幕/摄像头/画布功能以及 `system.run`
功能，而网关则保留在云端。
