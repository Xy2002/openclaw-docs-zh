...
- `/status` → **富含表情符号的状态卡片**，显示会话使用的模型、上下文使用量、最近一次回复的输入/输出令牌数，以及**预估成本**（仅限提供 API 密钥时）。
- `/usage off|tokens|full` → 在每条回复末尾添加一个**每条回复的使用情况页脚**。
  - 持久保存于会话中（存储为 `responseUsage`）。
  - OAuth 身份验证会**隐藏成本**（仅显示令牌数）。
...
这些是针对 `input`、`output`、`cacheRead` 和 `cacheWrite` 的**每 100 万令牌的美元价格**。如果缺少定价信息，OpenClaw 将仅显示令牌数。OAuth 令牌始终不显示美元成本。
...
您可在 [网关配置](/gateway/configuration)中进行相关设置，并在 [会话修剪](/concepts/session-pruning)中查看详细行为说明。
