...
macOS 应用程序是 OpenClaw 的**菜单栏伴侣**。它负责权限管理，
在本地管理或连接到网关（通过 launchd 或手动启动），并将 macOS 功能作为节点暴露给代理。
...
应用程序管理一个按用户划分的 LaunchAgent，其标签为 `bot.molt.gateway`
（当使用 `--profile`/`OPENCLAW_PROFILE` 时为 `bot.molt.<profile>`；旧版 `com.openclaw.*` 仍会被卸载）。
...
`system.run` 受 macOS 应用程序中**Exec 批准**的控制（设置 → Exec 批准）。
安全、询问和白名单信息本地存储在 Mac 上：
