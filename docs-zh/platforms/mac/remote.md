...
此流程使 macOS 应用程序能够作为在另一台主机（桌面或服务器）上运行的 OpenClaw 网关的完整远程控制工具。这是应用程序的 **通过 SSH 远程**（远程执行）功能。所有功能——健康检查、语音唤醒转发和 Web 聊天——都复用 *设置 → 通用* 中相同的远程 SSH 配置。

## 模式
- **本地（本 Mac）**：一切都在笔记本电脑上运行，不涉及 SSH。
- **通过 SSH 远程（默认）**：OpenClaw 命令在远程主机上执行。mac 应用程序会使用 `-o BatchMode` 加上您选择的身份/密钥以及本地端口转发来建立 SSH 连接。
- **直接远程（ws/wss）**：无需 SSH 隧道。mac 应用程序直接连接到网关 URL（例如，通过 Tailscale Serve 或公共 HTTPS 反向代理）。
...
2) 确保 `openclaw` 在非交互式 shell 的 PATH 中可用（必要时将其符号链接到 `/usr/local/bin` 或 `/opt/homebrew/bin`）。
3) 使用密钥认证启用 Open SSH。我们建议使用 **Tailscale** IP，以确保在局域网外也能稳定访问。

## macOS 应用程序设置
1) 打开 *设置 → 通用*。
2) 在 **OpenClaw 运行** 下，选择 **通过 SSH 远程** 并进行以下设置：
   - **传输方式**：**SSH 隧道** 或 **直接（ws/wss）**。
   - **SSH 目标**：`user@host`（可选 `:port`）。
     - 如果网关位于同一 LAN 并广播 Bonjour，则从发现的列表中选择它以自动填充此字段。
   - **网关 URL**（仅直接模式）：`wss://gateway.example.ts.net`（或 `ws://...` 用于本地/LAN）。
   - **身份文件**（高级）：您的密钥路径。
...
- **Web 聊天卡住**：确认网关正在远程主机上运行，并且转发的端口与网关的 WS 端口匹配；UI 需要健康的 WS 连接。
