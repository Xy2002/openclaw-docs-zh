...
- 如果您希望提供 Gatekeeper 安全的 DMG/ZIP 分发，需准备 Notary 凭证（钥匙串配置文件或 API 密钥）用于 `xcrun notarytool`。
  - 我们使用名为 `openclaw-notary` 的钥匙串配置文件，它基于 App Store Connect API 密钥环境变量在您的 shell 配置文件中创建：
...
- 使用 `scripts/package-mac-dist.sh` 生成发布工件（ZIP + DMG + Notarization）。使用 `scripts/package-mac-app.sh` 进行本地或开发环境的打包。
...
```
该工具从 `CHANGELOG.md` 生成 HTML 格式的发布说明（通过 [`scripts/changelog-to-html.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/changelog-to-html.sh)），并将这些说明嵌入到 appcast 条目中。在发布时，将更新后的 `appcast.xml` 与发布资产（ZIP + dSYM）一同提交。
...
- 健全性检查：
  ...
  - 在先前的公开版本上，从“关于”选项卡运行“检查更新…”，并验证 Sparkle 是否能顺利安装新版本。
