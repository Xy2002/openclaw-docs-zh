...
### 在授予权限时应用崩溃
如果您尝试允许“语音识别”或“麦克风”访问权限时应用崩溃，可能是 TCC 缓存损坏或签名不匹配所致。
...
1. 重置 TCC 权限：
   ...
   ```
2. 如果上述方法无效，请暂时在 [`scripts/package-mac-app.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`，以强制 macOS 生成一个“全新状态”。
...
```
如果某个手动运行的进程占用了端口，请停止该进程（按 Ctrl+C）。作为最后手段，您可以终止上面找到的 PID。
