...
如果你希望以大约 5 美元的成本实现“OpenClaw 全天候运行”，这是最简单且可靠的部署方案。Hetzner 的定价可能会变化，请选择最小的 Debian/Ubuntu VPS，如果遇到内存不足问题再进行扩容。
...
- 将 `~/.openclaw` 和 `~/.openclaw/workspace` 持久化到宿主机上（在重启或重建后仍能保留）
