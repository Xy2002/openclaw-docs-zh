...
- 复制 **状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.openclaw/`）——其中包括配置、身份验证、会话和频道状态。
- 复制您的 **工作区**（默认为 `~/.openclaw/workspace/`）——其中包括代理文件（记忆、提示等）。
...
在输出中查找对 `OPENCLAW_STATE_DIR` 或配置文件的引用。如果您运行多个网关，请为每个配置文件重复此步骤。
...
在 **新** 机器上，安装 CLI（如果需要，还需安装 Node.js）：
...
Doctor 是一个“安全且稳妥”的命令。它可以修复服务、应用配置迁移，并警告潜在的不匹配问题。
...
请务必迁移整个 `$OPENCLAW_STATE_DIR` 文件夹。
