...
- `https://openclaw.bot/install.sh` — “推荐”安装程序（默认全局 npm 安装；也可从 GitHub 代码库安装）
...
- 默认启用 `sharp`，以缓解原生安装中的陷阱，并默认设置 `SHARP_IGNORE_GLOBAL_LIBVIPS=1`（避免针对系统 libvips 进行构建）。
...
如果您在**已位于 OpenClaw 源代码检查目录内**时运行安装程序（通过 `package.json` + `pnpm-workspace.yaml` 检测到），它会提示您：
...
对于 `npm` 安装，Git*通常*不是必需的，但某些环境仍然需要 Git（例如，当某个包或依赖项通过 Git URL 获取时）。目前，安装程序会确保 Git 存在，以避免在全新发行版上出现 `spawn git ENOENT` 意外。
...
此脚本将 `openclaw` 安装到一个前缀中（默认：`~/.openclaw`），并在该前缀下安装一个专用的 Node 运行时，以便在您不想触碰系统 Node/npm 的机器上也能正常工作。
