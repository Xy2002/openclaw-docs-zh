...
OpenClaw 目前正处于快速迭代阶段（尚未达到“1.0”版本）。请将更新视为基础设施部署：更新 → 运行检查 → 重启（或使用 `openclaw update`，它会自动重启）→ 验证。

## 推荐：重新运行网站安装程序（就地升级）

**推荐**的更新方式是从网站重新运行安装程序。它能够检测现有安装、进行就地升级，并在必要时运行 `openclaw doctor`。
...
- 对于**源码安装**，请使用：
  ...
  ```
  安装程序**仅在仓库状态干净时**才会执行 `git pull --rebase`。
- 对于**全局安装**，脚本在后台使用 `npm install -g openclaw@latest`。
- 旧版说明：`openclaw` 仍作为兼容性适配层提供。

## 在更新之前

- 确认您的安装方式：是**全局安装**（npm/pnpm）还是**源码安装**（git 克隆）。
- 确认您的 Gateway 是以**前台终端**运行，还是通过**受管服务**（launchd/systemd）运行。
- 备份您的自定义配置：
...
```
我们**不建议**在 Gateway 运行时使用 Bun（WhatsApp/Telegram 存在相关问题）。
...
注意事项：
- 如果您的 Gateway 以服务形式运行，优先使用 `openclaw gateway restart`，而非直接杀死 PID。
- 如果您已固定到特定版本，请参阅下方的“回滚/固定”部分。

## 更新（`openclaw update`）

对于**源码安装**（git 检出），推荐使用以下命令：
...
- 要求工作树状态干净。
- 切换到选定的通道（标签或分支）。
- 获取并基于配置的上游分支进行变基（开发通道）。
- 安装依赖项、构建代码、构建 Control UI，并运行 `openclaw doctor`。
- 默认情况下会重启 Gateway（使用 `--no-restart` 可跳过重启）。

如果您通过 **npm/pnpm** 安装（无 git 元数据），`openclaw update` 将尝试通过您的包管理器进行更新。如果无法检测到安装，请改用“更新（全局安装）”。

## 更新（Control UI / RPC）

Control UI 提供了“更新与重启”功能（RPC：`update.run`）。其工作流程如下：
1) 执行与 `openclaw update` 相同的源码更新流程（仅适用于 git 检出）。
2) 写入重启标记，并附带结构化报告（stdout/stderr 尾部）。
3) 重启 Gateway，并使用报告向最近活跃的会话发送 ping。

如果变基失败，Gateway 将中止并重启，而不应用更新。
...
- 如果您从仓库检出运行且未进行全局安装，可使用 `pnpm openclaw ...` 来执行 CLI 命令。
- 如果您直接从 TypeScript 运行（`pnpm openclaw ...`），通常无需重新构建，但**配置迁移仍然适用** → 请运行 doctor。
- 在全局安装和 git 安装之间切换非常容易：安装另一种形式的软件，然后运行 `openclaw doctor`，以便 Gateway 服务入口点被重写为当前安装。

## 始终运行：`openclaw doctor`

Doctor 是“安全更新”命令。它的设计刻意简单：修复 + 迁移 + 警告。

注意：如果您使用的是**源码安装**（git 检出），`openclaw doctor` 会先提示您运行 `openclaw update`。

典型操作包括：
- 迁移已弃用的配置键或旧版配置文件位置。
- 审计 DM 政策，并对存在风险的“开放”设置发出警告。
- 检查 Gateway 的健康状况，并可能建议重启。
- 检测并迁移旧版 Gateway 服务（launchd/systemd；旧版 schtasks）到当前的 OpenClaw 服务。
- 在 Linux 上，确保 systemd 用户 linger 功能启用（以便 Gateway 在用户注销后仍能存活）。
...
运行手册及精确的服务标签：[Gateway 运行手册](/gateway)
...
选择某个日期的提交（例如：“截至 2026 年 1 月 1 日的 main 分支状态”）：
...
- 再次运行 `openclaw doctor`，并仔细阅读输出内容（它通常会提示您如何修复问题）。
