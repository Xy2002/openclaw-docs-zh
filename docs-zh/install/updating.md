...
**推荐**的更新路径是从网站重新运行安装程序。它能够检测现有安装、进行就地升级，并在必要时运行 `openclaw doctor`。
...
- 对于**源码安装**，请使用：
  ...
  ```
  安装程序**仅在代码库干净时**才会执行 `git pull --rebase`。
- 对于**全局安装**，脚本在后台使用 `npm install -g openclaw@latest`。
- 旧版说明：`openclaw` 仍作为兼容性适配层提供。

## 在更新之前

- 确认您的安装方式：是**全局安装**（npm/pnpm）还是**源码安装**（git clone）。
...
```
我们**不推荐**在 Gateway 运行时使用 Bun（WhatsApp/Telegram 存在相关问题）。
...
注意事项：
- 如果您的 Gateway 以服务形式运行，建议使用 `openclaw gateway restart` 而不是直接杀死 PID。
- 如果您已固定到特定版本，请参阅下方的“回滚/固定”部分。

## 更新（`openclaw update`）

对于**源码安装**（git checkout），推荐使用以下命令：
...
- 要求工作树保持干净。
- 切换到所选通道（标签或分支）。
- 拉取并基于配置的上游分支（开发通道）进行变基。
- 安装依赖、构建、编译 Control UI，并运行 `openclaw doctor`。
...
Control UI 提供“更新与重启”功能（RPC：`update.run`）。其工作流程如下：
1) 执行与 `openclaw update` 相同的源码更新流程（仅适用于 git checkout）。
2) 写入重启标记，并附带结构化报告（stdout/stderr 尾部）。
3) 重启 Gateway，并使用报告向最近活跃的会话发送 ping。

如果变基失败，Gateway 将中止并重启，但不会应用更新。
...
- 如果您从仓库检出运行且未进行全局安装，请使用 `pnpm openclaw ...` 来执行 CLI 命令。
...
注意：如果您使用的是**源码安装**（git checkout），`openclaw doctor` 会先提示您运行 `openclaw update`。
...
- 再次运行 `openclaw doctor`，并仔细阅读输出内容（它通常会给出解决方案）。
