...
如果你能够成功运行 `npm install -g openclaw@latest`，但随后看到 `openclaw: command not found`，这几乎总是由 **PATH** 问题引起：npm 将全局二进制文件放置的目录未包含在你的 shell 的 PATH 中。
...
如果 `npm install -g ...` 在出现 `EACCES` 的情况下失败，请将 npm 的全局前缀切换到用户可写目录：
...
为了让 Node 和 npm 的安装尽可能减少意外情况，建议采用以下方式：
...
如果你使用版本管理器（nvm/fnm/asdf 等），请确保它已在你日常使用的 shell（zsh 或 bash）中初始化，以便当你运行安装程序时，它所设置的 PATH 已生效。
