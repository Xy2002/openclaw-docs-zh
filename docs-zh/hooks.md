...
只要你会编写一个小型 TypeScript 函数，你就能编写一个钩子。钩子会自动发现，你可以通过 CLI 启用或禁用它们。
...
在引导过程中（`openclaw onboard`），系统会提示您启用推荐的钩子。向导会自动发现符合条件的钩子并将其呈现给您选择。
...
- **`emoji`**：用于 CLI 的显示表情符号（如 `"💾"`）
- **`events`**：要监听的事件数组（如 `["command:new", "command:reset"]`）
