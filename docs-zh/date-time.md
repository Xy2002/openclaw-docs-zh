...
如果已知用户时区，系统提示将包含一个专门的**当前日期与时间**部分，其中仅显示**时区**（不包含具体时间或时间格式），以确保提示缓存的稳定性：
...
插入到代理上下文中的排队系统事件会在开头加上时间戳，所使用的时区选择与消息信封相同（默认为主机本地时间）。
...
渠道工具返回**提供程序原生时间戳**，并添加规范化字段以确保一致性：

- `timestampMs`：UTC 的毫秒级纪元时间
- `timestampUtc`：UTC 的 ISO 8601 字符串

原始提供程序字段会被保留，以确保数据不丢失。
