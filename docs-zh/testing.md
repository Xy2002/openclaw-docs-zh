...
当调试真实的提供商/模型时（需要真实凭据）：
- 实时测试套件（模型 + 网关工具/镜像探针）：`pnpm test:live`

提示：如果你只需要一个失败用例，建议通过下面描述的白名单环境变量来缩小实时测试的范围。

## 测试套件（各套件的运行内容）

可以将这些套件视为“现实性逐渐增强”（同时故障率和成本也逐渐增加）：
...
- 预期：
  - 本质上并不适合 CI 稳定运行（真实网络、真实提供商策略、配额、宕机等）
  - 需要付费并受速率限制影响
  - 建议运行缩小后的子集，而不是“全部”
...
请参考以下决策表：
- 编辑逻辑/测试代码：运行 `pnpm test`（如果你做了大量更改，还应运行 `pnpm test:coverage`）
- 修改网关网络/WS 协议/配对：添加 `pnpm test:e2e`
- 调试“我的机器人挂了”/特定于提供商的故障/工具调用问题：运行缩小后的 `pnpm test:live`
...
- 设计目的：
  - 将“提供商 API 出现问题或密钥无效”与“网关代理流程出现问题”区分开来
...
- 探针细节（以便你能快速解释失败原因）：
  - `read` 探针：测试在工作区写入一个随机文件，并要求代理将其 `read` 并将随机数回显。
  - `exec+read` 探针：测试要求代理将随机数 `exec` 写入临时文件，然后将其 `read` 回读。
...
- 工具和图像探针在此实时测试中始终开启：
  ...
  - 流程（高层次）：
    - 测试生成一个带有“CAT”和随机代码的小型 PNG (`src/gateway/live-image-probe.ts`)
    - 通过 `agent` 和 `attachments: [{ mimeType: "image/png", content: "<base64>" }]` 发送
...
- 目标：使用本地 CLI 后端验证网关 + 代理流程，而不触及你的默认配置。
- 启用：
  - `pnpm test:live`（或如果你直接调用 Vitest，则使用 `OPENCLAW_LIVE_TEST=1`）
...
- Gemini API vs Gemini CLI：
  - API：OpenClaw 通过 HTTP 调用 Google 托管的 Gemini API（API 密钥 / 配置文件身份验证）；这正是大多数用户所说的“Gemini”。
  - CLI：OpenClaw 调用本地 `gemini` 二进制文件；它有自己的身份验证机制，并且行为可能不同（流式传输/工具支持/版本差异）。
...
- 如果实时测试显示“没有凭据”，请按照调试 `openclaw models list` / 模型选择的相同方式来进行排查。
...
这些运行 `pnpm test:live` 在仓库的 Docker 映像中，挂载你的本地配置目录和工作区（并在挂载时获取 `~/.profile`）：
...
我们已经有一些 CI 安全的测试，类似于“代理可靠性评估”：
- 通过真实网关 + 代理循环进行模拟工具调用（`src/gateway/gateway.tool-calling.mock-openai.test.ts`）。
- 端到端向导流程，验证会话布线和配置效果（`src/gateway/gateway.wizard.e2e.test.ts`）。

技能方面仍需完善（参见 [Skills](/tools/skills))：
- **决策能力：**当提示中列出技能时，代理是否会选择正确的技能（或避免无关技能）？
- **合规性：**代理在使用前是否读取 `SKILL.md` 并遵循必要的步骤/参数？
- **工作流契约：**多轮场景，确认工具顺序、会话历史继承和沙盒边界。

未来的评估应首先保持确定性：
- 使用模拟提供商的场景运行器，确认工具调用 + 顺序、技能文件读取和会话布线。
- 一套小型的技能聚焦场景（使用 vs 避免、门控、提示注入）。
- 可选的实时评估（可选、环境控制）仅在 CI 安全套件就绪后进行。
...
- 尽量针对能够捕捉该缺陷的最小层次：
  - 提供商请求转换/重放错误 → 直接模型测试
  - 网关会话/历史/工具流程错误 → 网关实时冒烟测试或 CI 安全的网关模拟测试
