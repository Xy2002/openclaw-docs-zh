...
默认值：
- `interruptOnSpeech`: true
- `voiceId`: 回退到 `ELEVENLABS_VOICE_ID` / `SAG_VOICE_ID`（或在提供 API 密钥时使用第一个 ElevenLabs 语音）
- `modelId`: 未设置时默认为 `eleven_v3`
- `apiKey`: 回退到 `ELEVENLABS_API_KEY`（或在可用时使用网关 Shell 配置文件）
- `outputFormat`: 在 macOS/iOS 上默认为 `pcm_44100`，在 Android 上默认为 `pcm_24000`（设置 `mp3_*` 可强制使用 MP3 流式传输）

## macOS 用户界面
- 菜单栏切换按钮：**交谈**
- 配置选项卡：**交谈模式**组（语音 ID + 打断开关）
- 叠加层：
  - **聆听**：云图标随麦克风音量脉动
  - **思考**：下沉动画
  - **说话**：放射状环形动画
...
- `stability` 用于 `eleven_v3`，其有效性已针对 `0.0`、`0.5` 或 `1.0` 进行验证；其他模型接受 `0..1`。
- 如果设置了 `latency_tier`，则会针对 `0..4` 进行验证。
- 在 Android 上，支持 `pcm_16000`、`pcm_22050`、`pcm_24000` 和 `pcm_44100` 输出格式，以实现低延迟的 AudioTrack 流式传输。
