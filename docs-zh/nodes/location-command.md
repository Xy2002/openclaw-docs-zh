...
## 概括
- `location.get` 是一种节点命令（通过 `node.invoke`）。
- 默认关闭。
- 设置使用选择器：关闭 / 使用时 / 始终。
- 独立切换开关：精确位置。

## 为何使用选择器而非简单开关
操作系统权限是多层次的。我们可以在应用内暴露一个选择器，但最终的实际授权仍由操作系统决定：
- iOS/macOS：用户可在系统提示或设置中选择“使用时”或“始终”。应用可请求升级权限，但操作系统可能要求用户前往设置中调整。
- Android：后台位置权限是单独的权限；在 Android 10 及以上版本中，通常需要通过设置流程授予。
- 精确位置是独立的授权（iOS 14+ 的“精确”，Android 的“精细”与“粗略”）。

UI 中的选择器决定了我们请求的位置模式；实际授权则存储在操作系统设置中。
...
UI 行为：
- 选择 `whileUsing` 会请求前台位置权限。
- 选择 `always` 会先确保 `whileUsing` 已启用，然后请求后台位置权限（或在必要时引导用户前往设置）。
...
- `LOCATION_BACKGROUND_UNAVAILABLE`：应用处于后台，但仅允许“使用时”模式。
- `LOCATION_TIMEOUT`：无法及时修复。
- `LOCATION_UNAVAILABLE`：系统故障或无可用提供商。

## 后台行为（未来）
目标：即使节点处于后台，模型也能请求位置信息，但仅在以下条件下生效：
- 用户选择了“始终”模式。
- 操作系统授予了后台位置权限。
...
注意事项：
- iOS：需要“始终”权限和后台位置模式。静默推送可能会被限制；可能出现间歇性失败。
