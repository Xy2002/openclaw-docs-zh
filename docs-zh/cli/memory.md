...
管理语义记忆的索引和搜索。
由活跃内存插件提供（默认：`memory-core`；设置 `plugins.slots.memory = "none"` 以禁用）。
...
通用：
- `--agent <id>`：将作用域限定为单个代理（默认：所有已配置的代理）。
- `--verbose`：在探测和索引期间输出详细日志。

备注：
- `memory status --deep` 探测向量和嵌入的可用性。
- `memory status --deep --index` 如果存储脏，则执行重新索引。
- `memory index --verbose` 打印各阶段的详细信息（提供者、模型、来源、批次活动）。
