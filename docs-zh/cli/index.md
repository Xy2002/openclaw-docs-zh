...
颜色方案的权威来源：`src/terminal/palette.ts`（又称“龙虾缝线”）。
...
- `openclaw security audit --deep` — 尽力进行实时网关探测。
...
当存在任何向导标志时（`--non-interactive`、`--mode`、`--remote-url`、`--remote-token`），向导将自动运行。
...
- `--skip-health`
- `--skip-ui`
- `--node-manager <npm|pnpm|bun>`（推荐使用 pnpm；不推荐使用 bun 作为网关运行时）
- `--json`
...
### `config`
非交互式配置助手（获取/设置/取消设置）。运行 `openclaw config` 而不带子命令会启动向导。
...
- `channels status`：检查网关可达性和渠道健康状况（`--probe` 执行额外检查；使用 `openclaw health` 或 `openclaw status --deep` 进行网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印警告及建议的修复方法（然后引导您前往 `openclaw doctor`）。
- `channels logs`：从网关日志文件中显示最近的渠道日志。
- `channels add`：在未传递任何标志时进行向导式设置；标志切换到非交互式模式。
...
子命令：
- `skills list`：列出技能（默认，无子命令时）。
- `skills info <name>`：显示单个技能的详细信息。
- `skills check`：总结已满足和缺失的要求。
...
#### `agents add [name]`
添加一个新的隔离代理。除非传递了标志（或 `--non-interactive`），否则会运行引导向导；在非交互式模式下需要 `--workspace`。
...
### 使用跟踪
OpenClaw 可以在有 OAuth/API 凭证的情况下显示提供商的使用情况/配额。
...
- 如果没有匹配的凭证，使用情况将被隐藏。
- 更多详情：请参阅 [使用跟踪](/concepts/usage-tracking)。
...
- `gateway status` 还会在检测到时显示旧版或额外的网关服务（`--deep` 添加系统级扫描）。以个人姓名命名的 OpenClaw 服务被视为一流服务，不会被标记为“额外”。
...
注释：
- TTY 会话会呈现彩色、结构化的视图；非 TTY 回退到纯文本。
...
始终包括认证概览以及认证库中资料的 OAuth 到期状态。`--probe` 执行实时请求（可能会消耗令牌并触发速率限制）。
...
- `cron add`（别名：`create`；需要 `--name` 并且必须精确选择 `--at` | `--every` | `--cron`，以及精确选择 `--system-event` | `--message` 中的一个有效载荷）
- `cron edit <id>`（修补字段）
