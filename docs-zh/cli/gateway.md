...
注意事项：
- 默认情况下，除非在 `~/.openclaw/openclaw.json` 中设置了 `gateway.mode=local`，否则网关将拒绝启动。对于临时或开发环境的运行，请使用 `--allow-unconfigured`。
- 在没有身份验证的情况下绑定到环回以外的地址会被阻止（安全防护措施）。
- 当获得授权时，`SIGUSR1` 会触发进程内重启（启用 `commands.restart` 或使用 gateway 工具/配置 apply/update）。
- `SIGINT`/`SIGTERM` 处理程序会停止网关进程，但不会恢复任何自定义终端状态。如果你使用 TUI 或原始模式输入包装 CLI，在退出前请先恢复终端。
...
`gateway status` 显示网关服务（launchd/systemd/schtasks），并提供可选的 RPC 探测。
...
- 多播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域名（例如 `openclaw.internal.`），并设置拆分 DNS 和 DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
