...
身份验证通过 WebSocket 握手时的 `connect.params.auth`
（令牌或密码）强制执行。有关详细信息，请参阅 [网关配置](/gateway/configuration) 中的 `gateway.auth`。

安全提示：控制 UI 是一个 **管理员界面**（用于聊天、配置、执行审批）。请勿将其公开暴露。首次加载后，UI 会将令牌存储在 `localStorage` 中。建议使用 localhost、Tailscale Serve 或 SSH 隧道。

## 快速路径（推荐）

- 完成初始设置后，CLI 现会自动使用您的令牌打开仪表板，并打印相同的带令牌链接。
- 随时重新打开：`openclaw dashboard`（复制链接，如果可能则打开浏览器，若为无头环境则显示 SSH 提示）。
