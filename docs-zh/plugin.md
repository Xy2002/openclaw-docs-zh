...
插件只是一个**小型代码模块**，通过额外的功能（命令、工具和 Gateway RPC）扩展 OpenClaw。
...
- Google Antigravity OAuth（提供商身份验证）——作为 `google-antigravity-auth` 内置（默认禁用）
- Gemini CLI OAuth（提供商身份验证）——作为 `google-gemini-cli-auth` 内置（默认禁用）
- Qwen OAuth（提供商身份验证）——作为 `qwen-portal-auth` 内置（默认禁用）
- Copilot Proxy（提供商身份验证）——本地 VS Code Copilot Proxy 桥接；与内置的 `github-copilot` 设备登录不同（内置，默认禁用）
...
插件与 Gateway**在同一进程中运行**，因此应被视为可信代码。工具开发指南：[插件代理工具](/plugins/agent-tools)。
...
每个条目都会成为一个插件。如果包列出了多个扩展，则插件 ID 将成为 `name/<fileBase>`。

如果您的插件导入 npm 依赖项，请在该目录中安装它们，以便 `node_modules` 可用（`npm install` / `pnpm install`）。

### 渠道目录元数据

渠道插件可以通过 `openclaw.channel` 宣传入职元数据，并通过 `openclaw.install` 提供安装提示。这使得核心目录无需存储数据。
...
或者将 `OPENCLAW_PLUGIN_CATALOG_PATHS`（或 `OPENCLAW_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（以逗号/分号/`PATH` 分隔）。每个文件都应包含 `{ "entries": [ { "name": "@scope/pkg", "openclaw": { "channel": {...}, "install": {...} } } ] }`。
...
- 未知 `channels.<id>` 键是**错误**，除非插件清单声明了渠道 ID。
- 插件配置使用嵌入在 `openclaw.plugin.json` 中的 JSON Schema 进行验证（`configSchema`）。
- 如果插件被禁用，其配置将被保留，并发出**警告**。

## 插件槽位（互斥类别）

某些插件类别是**互斥的**（一次只能有一个处于活动状态）。使用 `plugins.slots` 选择哪个插件拥有该槽位：
...
如果多个插件声明 `kind: "memory"`，则只有选定的一个会被加载。其他插件将被禁用，并提供诊断信息。
...
- 插件管理的钩子会在 `openclaw hooks list` 中以 `plugin:<id>` 的形式显示。
...
注意事项：
- 配置应放在 `channels.<id>` 下（而非 `plugins.entries`）。
- `meta.label` 用于 CLI/UI 列表中的标签。
- `meta.aliases` 添加备用 ID，用于规范化和 CLI 输入。
- `meta.preferOver` 列出渠道 ID，当两者都配置时跳过自动启用。
- `meta.detailLabel` 和 `meta.systemImage` 使 UI 能够显示更丰富的渠道标签/图标。

### 逐步编写新的消息渠道

当您想要一个**新的聊天界面**（“消息渠道”），而不是模型提供商时，请使用此方法。模型提供商文档位于 `/providers/*` 下。
...
- `meta.preferOver` 允许插件替换另一个渠道（自动启用优先考虑它）。
...
加载插件（扩展目录或 `plugins.load.paths`），重启 Gateway，然后在您的配置中配置 `channels.<id>`。
...
- `acceptsArgs`：命令是否接受参数（默认：否）。如果为否且提供了参数，命令将不匹配，消息将传递给其他处理程序
...
- CLI 命令：采用 kebab 或 camel 命名法，但避免与核心命令冲突
...
- 配置键稳定性：作用域包被归一化为 `plugins.entries.*` 的**无作用域** ID。
