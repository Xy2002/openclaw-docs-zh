...
1) 安装并启动位于菜单栏的**OpenClaw.app**。
2) 完成引导流程和权限检查清单（TCC提示）。
3) 确保Gateway处于**本地**模式并正在运行（由应用程序管理）。
4) 关联各表面（例如WhatsApp）：
...
- 先运行 `openclaw setup`，再运行 `openclaw channels login`，然后手动启动网关（`openclaw gateway`）。
...
- [网关配置](/gateway/configuration)（配置模式与示例）
