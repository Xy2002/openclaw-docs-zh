...
本文档描述了**当前**的首次运行入门流程。其目标是提供流畅的“第 0 天”体验：让用户轻松选择网关的运行位置、完成身份验证连接、运行向导，并让代理自动完成引导。
...
- **本地（本 Mac）：** 入门流程可以执行 OAuth 流程，并将凭据本地写入。
- **远程（通过 SSH/Tailnet）：** 入门流程**不会**在本地执行 OAuth；凭据必须存储在网关主机上。
...
该应用可以运行与 CLI 相同的设置向导。这确保了入门流程与网关端的行为保持一致，并避免在 SwiftUI 中重复逻辑。
...
该应用可通过 npm 或 pnpm 全局安装 `openclaw` CLI，从而让终端工作流和 launchd 任务开箱即用。

6）入门聊天（专用会话）

设置完成后，应用会打开一个专用的入门聊天会话，让客服人员自我介绍并指导您完成后续步骤。这样可以将首次运行的引导信息与您的正常对话区分开来。
