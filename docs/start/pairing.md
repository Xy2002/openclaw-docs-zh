...
- 8个字符，全为大写，不含易混淆字符 (`0O1I`)。
- **1小时后过期**。只有在创建新请求时（大致每小时每个发件人一次），机器人才会发送配对消息。
- 默认情况下，每个频道最多保留**3个待处理的DM配对请求**；在其中一个请求过期或被批准之前，额外的请求将被忽略。
...
- 旧版 `node.pair.*`API（CLI：`openclaw nodes pending/approve`）是由网关独立管理的配对存储。WS 节点仍然需要进行设备配对。
