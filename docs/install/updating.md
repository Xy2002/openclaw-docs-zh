...
- 了解您的安装方式：**全局安装**（npm/pnpm）还是**源码安装**（git clone）。
- 了解您的 Gateway 运行方式：**前台终端**还是**受监督的服务**（launchd/systemd）。
- 备份您的自定义配置：
...
Doctor 是“安全更新”命令。它设计得非常简单：修复 + 过渡 + 警告。
