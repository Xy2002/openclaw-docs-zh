...
- `ItemParam` 是一个区分联合类型，包括：
  ...
  - `reasoning`
  - `item_reference`
- 成功响应返回一个包含 `ResponseResource` 的对象，其中包含 `object: "response"`、`status` 和 `output` 项。
- 流式传输使用语义事件，例如：
...
- 当 Chat Completions 启用时发出启动警告，以提示其为遗留功能。
...
- 使用 `invalid_request_error` 拒绝不受支持的内容部分（如图像/文件）。
- 返回一条助手消息，内容为 `output_text`。
- 返回 `usage`，其中值置零，直到令牌计数功能完成对接。
