...
- 在此之前（Bun 路径）， `openclaw status` 和 `gateway:watch` 运行正常。

## 临时解决方案

- 在开发脚本中继续使用 Bun（当前的临时回滚方案）。
- 使用 Node 和 tsc watch 编译代码，然后运行编译后的输出：
...
- 如果已知存在回归问题，可尝试使用 nightly 版本的 `tsx`，或锁定到较早版本进行测试。
- 如果问题在 Node LTS 上也能复现，则应向上游提交包含 `__name` 堆栈跟踪的最小复现方案。
