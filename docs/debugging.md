...
在聊天中使用 `/debug` 可设置**仅运行时生效**的配置覆盖（存储于内存，而非磁盘）。默认情况下 `/debug` 处于禁用状态；可通过 `commands.debug: true` 启用。当您需要在不编辑 `openclaw.json` 的情况下切换某些晦涩设置时，此功能非常有用。
...
使用开发配置文件可隔离状态，并为调试启动一个安全、可丢弃的环境。这里有**两个** `--dev` 标志：
...
1) **配置文件隔离**（全局 `--dev`）
   ...
   - `OPENCLAW_GATEWAY_PORT=19001`（浏览器/画布随之调整）
...
注意：`--dev` 是一个**全局**配置文件标志，可能会被某些运行程序吃掉。如果需要明确指定，可以使用环境变量形式：
