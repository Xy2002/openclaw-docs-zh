...
OpenClaw可以在你已使用的任何聊天应用中与你对话。每个频道都通过网关连接。
...
- 各频道可同时运行；配置多个频道后，OpenClaw会根据聊天内容自动路由消息。
- 通常情况下，设置最快的渠道是**Telegram**（只需提供简单的机器人令牌即可）。WhatsApp需要通过二维码进行配对，并且会在磁盘上存储更多状态信息。
- 不同频道的群组行为各不相同；有关详细信息，请参阅[群组](/concepts/groups)。
- 为确保安全，系统会强制执行私信配对和白名单机制；有关详细信息，请参阅[安全](/gateway/security)。
- Telegram内部机制：[grammY备忘录](/channels/grammy)。
- 故障排除：[频道故障排除](/channels/troubleshooting)。
- 模型提供商的相关信息将另行说明；有关详细信息，请参阅[模型提供商](/providers/models)。
