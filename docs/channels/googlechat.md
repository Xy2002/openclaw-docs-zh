...
---
__HEADING_0__Google Chat（Chat API）
...
- 填写**应用信息**：
   ...
   - 在**连接设置**中，选择**HTTP终端点URL**。
   - 在**触发器**部分，选择**对所有触发器使用通用HTTP终端点URL**，并将其设置为您的网关公共URL后加上 `/googlechat`。
     - *提示：运行 `openclaw status` 以查找您的网关的公共URL。*
   - 在**可见性**部分，勾选**使此Chat应用对&lt;您的域名&gt;中的特定人员和群组可用**。
...
- **注意**：由于此机器人是私有应用，它不会出现在“市场”浏览列表中。您必须按名称搜索它。
...
Google Chat Webhook 需要一个公共 HTTPS 端点。出于安全考虑，**仅将 `/googlechat` 路径暴露给互联网**。请将 OpenClaw 仪表板及其他敏感端点保留在您的私有网络中。

### 方案A：Tailscale Funnel（推荐）
...
如果出现提示，请访问输出中显示的授权URL，以在您的尾网策略中为此节点启用漏斗功能。
...
使用此配置，任何对 `your-domain.com/` 的请求都将被忽略或返回404，而 `your-domain.com/googlechat` 则会安全地路由到 OpenClaw。
