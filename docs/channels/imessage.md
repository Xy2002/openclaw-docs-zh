...
如果您希望机器人使用 **单独的 iMessage 身份** 发送消息（并保持您的个人“信息”干净），请使用专用 Apple ID + 专用 macOS 用户。
...
首次运行注意事项：发送/接收可能需要在 *机器人 macOS 用户* 中进行 GUI 批准（自动化 + 全盘访问权限）。如果 `imsg rpc` 似乎卡住或退出，请登录该用户（屏幕共享有助于操作），运行一次性 `imsg chats --limit 1` / `imsg send ...`，批准提示，然后重试。
...
**远程附件：** 当 `cliPath` 通过 SSH 指向远程主机时，“信息”数据库中的附件路径引用远程机器上的文件。通过设置 `channels.imessage.remoteHost`，OpenClaw 可以通过 SCP 自动获取这些文件：
...
多账户支持：使用 `channels.imessage.accounts` 结合每账户配置和可选 `name`。共享模式请参见 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。不要提交 `~/.openclaw/openclaw.json`（其中通常包含令牌）。
...
```
当您希望为特定线程提供隔离的个性/模型时，这非常有用（参见 [多代理路由](/concepts/multi-agent))。有关文件系统隔离，请参见 [沙箱化](/gateway/sandboxing)。
...
- `channels.imessage.chunkMode`：`length`（默认）或 `newline` 以在长度分块前按空行（段落边界）分割。
