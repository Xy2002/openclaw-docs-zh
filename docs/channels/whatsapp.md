...
---
__HEADING_0__WhatsApp（网页渠道）

状态：仅通过Baileys使用WhatsApp Web。网关拥有会话。
...
- 在一个网关进程中支持多个WhatsApp账户（多账户）。
- 确定性路由：回复直接返回到WhatsApp，不经过模型路由。
- 模型能够获取足够的上下文，以理解引用回复。
...
为 OpenClaw 使用**独立的手机号码**。这样可带来最佳用户体验，路由清晰，且不存在自聊等异常行为。理想的设置是：**备用或旧安卓手机 + eSIM**。将设备始终连接到 Wi-Fi 并保持通电状态，然后通过二维码完成绑定。

**WhatsApp Business：** 您可以在同一设备上使用不同的号码运行 WhatsApp Business。这对于将个人 WhatsApp 与工作分开非常方便——只需安装 WhatsApp Business，并在其中注册您的 OpenClaw 号码。
...
当向导询问您的个人 WhatsApp 号码时，请输入您将用于发送消息的电话号码（所有者或发件人），而不是助手号码。
...
在未设置`messages.responsePrefix`的情况下，自聊回复在设置时默认为`[{identity.name}]`（否则为`[openclaw]`）。可通过显式设置来自定义或禁用前缀（使用`""`可移除前缀）。
...
- **预付费 SIM**— 价格低廉，只需接收一条短信即可完成验证

**避免使用：** TextNow、Google Voice 以及大多数“免费短信”服务——WhatsApp 对这些服务实施了严格的封锁措施。

**提示：** 号码只需接收一条验证短信。之后，WhatsApp Web 会话可通过 `creds.json` 持续保持在线。

## 为什么不用 Twilio？

- 早期的 OpenClaw 版本支持通过 Twilio 集成 WhatsApp Business。
- WhatsApp Business 号码并不适用于个人助理场景。
- Meta 强制要求在24小时内回复消息；如果在过去24小时内未回复，该商业号码将无法主动发起新消息。
- 在高流量或“聊天式”使用场景下，商业账户会因被判定为发送大量个人助理消息而遭到严厉封禁，因为此类账户并非为此类用途设计。
- 结果：由于交付不可靠且频繁被封禁，我们已停止对该功能的支持。
...
- 收件箱监听器在关闭时会断开连接，以避免在测试或重启过程中累积事件处理程序。
- 状态和广播聊天会被忽略。
- 直接聊天采用 E.164 格式；群组使用群组 JID。
- **私信策略**：`channels.whatsapp.dmPolicy` 控制直接聊天的访问权限（默认：`pairing`）。
  - 配对模式：未知发件人会收到配对代码（通过 `openclaw pairing approve whatsapp <code>` 批准；代码 1 小时后失效）。
  - 开放模式：需要 `channels.whatsapp.allowFrom` 并包含 `"*"`。
  - 您已绑定的 WhatsApp 号码被视为隐含信任，因此来自该号码的消息会跳过 `channels.whatsapp.dmPolicy` 和 `channels.whatsapp.allowFrom` 检查。
...
默认情况下，网关会在接收入站 WhatsApp 消息后立即将其标记为已读（显示蓝勾）。
...
__HEADING_0__WhatsApp常见问题解答：发送消息 + 配着

**当我绑定 WhatsApp 时，OpenClaw 会随机给联系人发送消息吗？**  
不会。默认的私信策略是“配着”，因此未知发件人只会收到配着代码，他们的消息**不会被处理**。OpenClaw 只会回复它收到的聊天消息，或者您明确通过代理或 CLI 触发的发送。
...
WhatsApp可以在收到传入消息后立即自动发送表情符号反应，在机器人生成回复之前提供即时反馈，让用户知道他们的消息已被接收。
...
WhatsApp以**语音笔记**（PTT气泡）的形式发送音频。

- 最佳效果：OGG/Opus。OpenClaw会将`audio/ogg`重写为`audio/ogg; codecs=opus`。
- `[[audio_as_voice]]`对WhatsApp无效（音频已以语音笔记形式发送）。

## 媒体限制 + 优化

- 默认出站上限：5 MB（每件媒体）。
- 覆盖：`agents.defaults.mediaMaxMb`。
- 在上限范围内，图片会自动优化为 JPEG（调整大小 + 质量扫描）。
- 超大媒体会导致错误；媒体回复将降级为文本警告。

## 心跳

- **网关心跳**记录连接健康状况（`web.heartbeatSeconds`，默认 60 秒）。
- **代理心跳**可按代理配置（`agents.list[].heartbeat`）设置，或通过 `agents.defaults.heartbeat` 进行全局配置（在未设置代理特定条目时的回退）。
...
- `channels.whatsapp.selfChatMode`（与手机设置相同；机器人使用您的个人 WhatsApp 号码）。
...
- 解决方法：在网关主机上运行 `openclaw channels login`，并扫描二维码（WhatsApp → 设置 → 已链接设备）。
...
- **不推荐使用 Bun**。WhatsApp（Baileys）和 Telegram 在 Bun 上运行不稳定。
