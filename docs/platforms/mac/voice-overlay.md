...
- 钟声和叠加日志在`info`处按`voicewake.overlay`、`voicewake.ptt`和`voicewake.chime`类别发出，分别对应会话开始、部分转录、最终转录、转录发送、会话关闭以及钟声触发原因。
...
2. 重构`VoiceWakeRuntime`，以通过创建、更新或结束会话来管理会话状态，而不是直接操作`VoiceWakeOverlayController`。
3. 重构`VoicePushToTalk`，使其采用现有会话，并在释放会话时调用`endCapture`；同时应用运行时冷却机制。
