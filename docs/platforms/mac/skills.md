...
## 数据源
- `skills.status`（网关）返回所有技能，以及其资格状态和缺失的要求（包括针对捆绑技能的白名单限制）。
...
- 当提供多个安装程序时，网关仅显示一个首选安装程序：如果有 brew，则优先使用 brew；否则使用来自 `skills.install` 的 node 管理器；默认为 npm。
