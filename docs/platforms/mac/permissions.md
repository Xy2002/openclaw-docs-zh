...
macOS的权限授予机制极其敏感。TCC将权限授予与应用的代码签名、捆绑包标识符以及磁盘路径紧密关联。只要上述任何一个要素发生变化，macOS就会将该应用视为全新应用，并可能丢弃或隐藏与其相关的权限提示。
...
如果您正在测试权限，请务必使用真实证书进行签名。临时签名构建仅适用于对权限要求不高、需要快速在本地运行的场景。
