...
macOS 权限授予非常脆弱。TCC 将权限授予与应用的代码签名、捆绑包标识符以及磁盘路径相关联。如果其中任何一项发生更改，macOS 会将该应用视为新应用，并可能丢弃或隐藏相关提示。

## 稳定权限的要求
- 相同路径：从固定位置运行应用（对于 OpenClaw，请使用 `dist/OpenClaw.app`）。
- 相同捆绑包标识符：更改捆绑包 ID 会创建新的权限身份。
- 已签名的应用：未签名或使用临时签名构建的应用无法持久保留权限。
- 稳定的签名：使用真实的 Apple 开发者证书或开发者 ID 证书，以确保在重新构建时签名保持稳定。

临时签名会在每次构建时生成新的身份。macOS 会忘记之前的权限授予，并且在清除过时条目之前，提示可能会完全消失。
...
如果您正在测试权限，请务必使用真实证书进行签名。临时签名构建仅适用于对权限无关紧要的快速本地运行场景。
