...
**安全提示：** 默认配置会暴露一个公共 URL。如需在无公共 IP 的情况下进行加固部署，请参阅 [私有部署](#private-deployment-hardened)，或使用 `fly.private.toml`。
...
- 请将这些令牌视为密码。
- 对于所有 API 密钥和令牌，**应优先使用环境变量而非配置文件**。这样可将密钥保存在 `openclaw.json` 之外，从而避免意外暴露或记录。
...
首次部署会构建 Docker 镜像（大约 2–3 分钟）。后续部署速度更快。
...
**注意：** 512MB 太小。1GB 或许勉强够用，但在高负载或日志记录较多的情况下仍可能出现内存不足（OOM）问题。**建议使用 2GB。**
...
如果您使用 `--allow-unconfigured`，网关将创建一个最小配置。您在 `/data/openclaw.json` 中的自定义配置应在重启时被读取。
...
如果在重启后丢失凭据或会话，说明状态目录正被写入容器文件系统。
...
如果您需要在不公开暴露的情况下实现Webhook回调（如Twilio、Telnyx等）：

1. **ngrok 隧道** - 在容器内运行 ngrok，或将其作为旁路容器运行
2. **Tailscale Funnel** - 通过 Tailscale 暴露特定路径
3. **仅出站** - 某些提供商（如 Twilio）即使没有 webhook 也能很好地处理出站调用
...
- Dockerfile 兼容两种架构
- 若要在 WhatsApp/Telegram 上线，请使用 `fly ssh console`
- 持久化数据存储在卷上的 `/data`
- Signal 需要 Java 和 signal-cli；请使用自定义镜像，并将内存保持在 2GB 以上。

## 成本

使用推荐配置（`shared-cpu-2x`, 2GB内存）：
