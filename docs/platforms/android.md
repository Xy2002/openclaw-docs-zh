...
Android 直接通过 WebSocket 连接到网关（默认 `ws://<host>:18789`），并使用由网关管理的配对机制。

### 先决条件

- 您可以在“主”机器上运行网关。
- Android 设备或模拟器可以通过以下方式访问网关 WebSocket：
  - 与网关位于同一局域网，并使用 mDNS/NSD；**或者**
  - 通过 Tailscale 尾网使用广域 Bonjour 或单播 DNS-SD（见下文）；**或者**
  - 手动指定网关主机和端口（备用方案）
- 您可以在网关机器上直接运行 CLI（`openclaw`），或通过 SSH 远程运行。
...
对于仅使用尾网的设置（推荐用于维也纳⇄伦敦），将网关绑定到尾网IP：
...
Android 的 NSD/mDNS 发现无法跨网络工作。如果您的 Android 节点和网关位于不同网络但通过 Tailscale 连接，则应改用广域 Bonjour 或单播 DNS-SD：
...
- 如果mDNS被阻止，请使用**高级 → 手动网关**（主机 + 端口），然后点击**手动连接**。
