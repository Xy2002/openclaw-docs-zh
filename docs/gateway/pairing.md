...
- **传输层**：网关 WS 端点会转发请求，但不决定成员资格。（旧版 TCP 桥接支持已弃用或移除。）
...
- `node.pair.resolved` — 在请求被批准/拒绝/过期时触发。
...
- 批准 **始终** 会生成一个新令牌；从未从 `node.pair.request` 返回任何现有令牌。
...
- 传输层是 **无状态的**；它不存储成员资格。
- 如果网关离线或配对功能被禁用，节点将无法配对。
- 如果网关处于远程模式，配 Pairing 仍会针对远程网关的存储进行。
