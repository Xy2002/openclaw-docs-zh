...
完整矩阵（范围、工作区挂载、镜像）请参见 [沙盒化](/gateway/sandboxing)。

### 绑定挂载（安全快速检查）

- `docker.binds` 会*穿透*沙盒文件系统：无论你挂载什么，在容器内都将以你设置的模式可见（`:ro` 或 `:rw`）。
...
- `/exec` 只会更改授权发送者的会话默认设置，并不授予工具访问权限。
...
- 提升**不**按技能范围划分，且**不会**覆盖工具的允许或拒绝规则。
