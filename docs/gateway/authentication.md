...
OpenClaw 支持为模型提供商使用 OAuth 和 API 密钥。对于 Anthropic 帐户，我们建议使用 **API 密钥**。若要访问 Claude 订阅功能，请使用由 `claude setup-token` 创建的长期有效令牌。
...
对于 Anthropic，推荐使用 **API 密钥**。如果您使用的是 Claude 订阅，也支持 setup-token 流程。请在 **网关主机** 上运行以下命令：
...
使用 `--agent <id>` 可指定特定代理；省略则使用已配置的默认代理。
