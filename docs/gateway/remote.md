...
- 对于**操作员（您或 macOS 应用程序）**：SSH 隧道是一种通用的后备方案。
- 对于**节点（iOS/Android 及未来设备）**：根据需要通过 **WebSocket** 连接到网关（可通过局域网/尾网或 SSH 隧道实现）。

## 核心理念

- 网关 WebSocket 绑定到您配置的端口上的**环回地址**（默认为 18789）。
- 在远程使用场景中，您可以通过 SSH 转发该环回端口（或者使用尾网/VPN 来减少对隧道的需求）。

## 常见VPN/尾网设置（代理所在位置）

将**网关主机**视为“代理所在的位置”。它负责管理会话、认证配置文件、通道和状态。您的笔记本电脑或台式机（以及节点）都连接到该主机。

### 1) 尾网中的常驻网关（VPS或家庭服务器）

在持久主机上运行网关，并通过 **Tailscale** 或 SSH 访问它。

- **最佳用户体验**：保持 `gateway.bind: "loopback"`，并使用 **Tailscale Serve** 提供控制界面。
- **后备方案**：使用环回，并从任何需要访问的机器建立 SSH 隧道。
- **示例**：[exe.dev](/platforms/exe-dev)（简易虚拟机）或 [Hetzner](/platforms/hetzner)（生产级 VPS）。

当您的笔记本电脑经常进入休眠状态，但您希望代理始终保持在线时，这是理想的选择。

### 2) 家庭桌面运行网关，笔记本电脑用作远程控制端
...
一个网关服务负责管理状态和通道。节点则是外围设备。

流程示例（Telegram → 节点）：

- Telegram消息到达**网关**。
- 网关运行**代理**，并决定是否调用节点工具。
- 网关通过网关WebSocket（`node.*`RPC）调用**节点**。
- 节点返回结果；网关再将响应发送回Telegram。
...
- 在需要时，`openclaw gateway {status,health,send,agent,call}` 也可以通过 `--url` 指向转发的 URL。
...
当网关仅监听环回地址时，请将 URL 保持为 `ws://127.0.0.1:18789`，并先建立 SSH 隧道。
...
- 在 macOS 上，建议使用应用程序的“通过 SSH 运行远程命令”模式，该模式可自动管理隧道。

## macOS 应用程序的“通过 SSH 进行远程”

macOS 菜单栏应用程序可以端到端地驱动同一套设置，包括远程状态检查、WebChat 和语音唤醒转发。

操作手册：[在 macOS 上进行远程访问](/platforms/mac/remote)。

## 安全规则（远程/VPN）

简要说明：**除非您确定需要绑定，否则请将网关限制为仅监听环回地址**。

- **环回 + SSH/Tailscale Serve** 是最安全的默认配置（无公开暴露）。
...
如果您希望使用令牌/密码，则将其设置为 `false`。

- 浏览器控制应被视为操作员访问：仅限尾网，并需明确进行节点配对。

深入解读：[安全](/gateway/security)。
