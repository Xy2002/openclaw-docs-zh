...
**目标（北极星）：** 在明确假设的基础上，提供经机器检查的安全论证，证明 OpenClaw 能够在其预期的安全策略（授权、会话隔离、工具门控以及错误配置安全性）下安全运行。
...
**尚非：** 这些模型并不能构成“OpenClaw 在所有方面都是安全的”这一结论的完整证明，也不意味着完整的 TypeScript 实现是完全正确的。
...
目前，可以通过在本地克隆模型仓库并运行TLC来重现结果（见下文）。未来的迭代可能包括：

- 在持续集成中运行模型，并公开相关工件（反例轨迹、运行日志）；
...
**断言：** 如果未启用认证而绑定到环回以外的地址，可能导致远程入侵或增加暴露风险；根据模型假设，令牌或密码可有效阻止未认证攻击者。
...
__HEADING_0__Nodes.run 管道（最高风险功能）

**断言：** `nodes.run` 要求：(a) 节点命令白名单应包含已声明的命令；(b) 在配置时需进行实时批准；批准过程通过令牌化来防止重放攻击（在模型中体现）。
...
**断言：** 不同对等方之间的DM不会在未显式关联或配置的情况下合并到同一会话中。
...
这些是后续模型，旨在更精确地模拟现实世界中的故障模式，包括非原子更新、重试以及消息扇出。
...
- 在并发请求下，某个通道上的 `MaxPending` 值不得被超过。
...
- 重试不会导致重复处理。
- 如果缺少提供商事件ID，去重机制将回退到一个安全键（如轨迹ID），以避免丢失不同事件。
...
**断言：** 路由默认应保持 DM 会话隔离，仅在显式配置时才合并会话（通道优先级 + 身份链接）。
