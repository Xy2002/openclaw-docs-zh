...
- **身份第一**：确定哪些用户可以与机器人对话，例如通过私信配对、加入白名单或明确将机器人设置为“开放”。
- **范围其次**：界定机器人被允许活动的范围，例如结合群组白名单与提及门控、工具、沙箱以及设备权限来加以限制。
- **模型最后**：假设模型可能遭到操纵；在设计时应确保任何操纵的影响范围都受到严格限制。
...
不同层级模型对提示注入的抵抗力并不一致。较小或较便宜的模型通常更容易受到工具滥用和指令劫持的影响，尤其是在面对恶意提示时。
