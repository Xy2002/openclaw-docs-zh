...
- `elevated`（布尔值）：如果已启用或允许提升模式，则在主机上运行
- 需要真正的TTY？设置`pty: true`。
- `workdir`、`env`

行为：
- 前台运行会直接返回输出。
- 当被转入后台（显式或因超时）时，该工具会返回`status: "running"` + `sessionId`以及一段简短的尾部输出。
- 输出会一直保留在内存中，直到会话被轮询或清除。
- 如果`process`工具被禁用，`exec`将同步运行，并忽略`yieldMs`/`background`。

## 子进程桥接

当在exec/process工具之外启动长时间运行的子进程时（例如CLI重新启动或网关助手），应附加子进程桥接助手，以便在退出或发生错误时转发终止信号并分离监听器。这可避免systemd中的孤儿进程，并确保跨平台的关机行为一致。
...
- `process`的作用域限定于单个代理；它仅能看到由该代理启动的会话。
