...
> **心跳 vs Cron？** 请参阅 [Cron与心跳](/automation/cron-vs-heartbeat)，以获取在何时使用每种方法的指导。

心跳在主会话中运行“定期代理回合”，使模型能够及时提醒您需要注意的事项，同时尽量避免频繁打扰您。
...
- **后台任务**：“考虑未完成的任务”会定期提醒代理检查后续事项（收件箱、日历、提醒、待处理工作），并及时提醒任何紧急事务。
