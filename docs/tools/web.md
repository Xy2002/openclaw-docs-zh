...
**环境变量替代方案：**在网关环境中设置 `OPENROUTER_API_KEY` 或 `PERPLEXITY_API_KEY`。对于网关安装，将其放入 `~/.openclaw/.env`。
...
- `web_fetch` 首先使用 Readability 提取主要内容，然后在已配置的情况下使用 Firecrawl。如果两者都失败，工具将返回错误。
- Firecrawl 请求默认启用规避机器人模式，并对结果进行缓存。
