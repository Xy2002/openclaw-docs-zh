...
- 如果智能体处于沙箱中，二进制文件也必须存在于**容器内**。通过 `agents.defaults.sandbox.docker.setupCommand`（或自定义镜像）安装它。
  ...
  软件包安装还需要网络出口、可写的根文件系统以及沙箱中的 root 用户。例如，`summarize` 技能（`skills/summarize/SKILL.md`）需要在沙箱容器中具有 `summarize` CLI 才能运行。
...
如果启用了技能监视器，或者出现了新的符合条件的远程节点，技能也可以在会话中途刷新（见下文）。您可以将其视为**热重载**：刷新后的列表将在下一个智能体回合中被采用。
...
這依賴於節點報告其命令支援，並通過 `system.run`進行二進制探測。如果 macOS 节点隨後離線，技能仍然可見；調用可能會失敗，直到節點重新連接。

## 技能監視器（自動刷新）
默認情況下，OpenClaw 會監視技能文件夾，並在 `SKILL.md` 文件發生變化時更新技能快照。您可以通過 `skills.load`進行配置：
...
## 令牌影響（技能列表）
當技能符合條件時，OpenClaw 會將一個緊湊的可用技能 XML 列表注入系統提示中（通過 `formatSkillsForPrompt` 在 `pi-coding-agent` 中）。成本是確定性的：

- **基礎開銷（僅當 ≥1 技能時）：** 195 个字符。
- **每項技能：** 97 个字符加上 XML 轉義的 `<name>`、`<description>` 和 `<location>` 值的长度。
...
- 不同模型的分詞器計算的令牌數量有所不同。粗略的 OpenAI 式估算約為 4 个字符/令牌，因此**每項技能約 97 个字符 ≈ 24 个令牌**，再加上您實際字段的長度。

## 托管技能生命周期
OpenClaw 隨安裝包（npm 包或 OpenClaw.app）提供一組基準技能作為**捆綁技能**。`~/.openclaw/skills` 用於本地覆蓋（例如，在不更改變捆綁副本的情況下修復或補丁某個技能）。工作區技能由用戶擁有，並在名稱衝突時覆蓋其他技能。

## 配置參考
有關完整的配置模式請參閱 [技能配置](/tools/skills-config)。

## 尋找更多技能？

請瀏覽 https://clawhub.com.
