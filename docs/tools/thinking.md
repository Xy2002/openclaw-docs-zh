...
- 层级（别名）：`off | minimal | low | medium | high | xhigh`（仅适用于 GPT-5.2 和 Codex 模型）
  ...
  - medium → “更深入地思考”
  - high → “极致思考”（最大预算）
  - xhigh → “极致思考+”（仅适用于 GPT-5.2 和 Codex 模型）
...
- 级别：`on`（极简）| `full` | `off`（默认）。
- 仅含指令的消息可切换会话的详细模式，并回复 `Verbose logging enabled.` / `Verbose logging disabled.`；无效级别将返回提示而不改变状态。
- `/verbose off` 存储显式会话覆盖；可通过 Sessions UI 选择 `inherit` 来清除。
- 内联指令仅影响该条消息；其他情况下适用会话或全局默认值。
- 发送 `/verbose`（或 `/verbose:`）且不带参数，即可查看当前的详细模式级别。
...
- Web聊天中的思维选择器在页面加载时会自动采用入站会话存储或配置中保存的会话层级。
- 选择其他层级仅对下一条消息生效（`thinkingOnce`）；发送该消息后，选择器会自动恢复为保存的会话层级。
- 若要更改会话默认值，可像以往一样发送 `/think:<level>` 指令；下次页面重新加载时，选择器将反映这一更改。
