...
**请勿**将您的凭据提供给模型。自动登录往往会触发反机器人防护机制，可能导致账户被锁定。

返回主浏览器文档：[浏览器](/tools/browser)。

## 使用哪个 Chrome 配置文件？

OpenClaw 控制一个**专用的 Chrome 配置文件**（名为 `openclaw`，界面带有橙色色调）。该配置文件与您日常使用的浏览器配置文件相互独立。
...
在沙箱环境中运行的浏览器会话**更有可能**触发机器人检测。对于X/Twitter等限制严格的网站，建议优先使用**主机**浏览器。
