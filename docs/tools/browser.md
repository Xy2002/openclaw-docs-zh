...
OpenClaw 可以运行一个由代理控制的**专用 Chrome/Brave/Edge/Chromium 配置文件**。该配置文件与您的个人浏览器完全隔离，并通过 Gateway 内部的一个小型本地控制服务进行管理（仅限环回）。
...
此浏览器**并非您的日常主力浏览器**，而是专为代理自动化和验证提供的一块安全、隔离的工作区域。
...
如果您看到“浏览器已禁用”，请在配置中将其启用（见下文）并重启网关。
...
如果您在已安装浏览器的机器上运行**节点主机**，OpenClaw 可以自动将浏览器工具调用路由到该节点，而无需额外的浏览器配置。这是远程网关的默认路径。
...
- **openclaw-managed**：一个专用的基于 Chromium 的浏览器实例，配备独立的用户数据目录和 CDP 端口。
...
- 将扩展固定到工具栏，然后在您想要控制的标签页上点击它（徽章显示 `ON`）。
...
可选：如果您需要不同的名称或中继端口，可以创建自己的配置文件：
...
在本地启动时，OpenClaw会按以下顺序选择第一个可用的浏览器：
...
仅适用于本地集成，Gateway公开了一个小型环回HTTP API：
...
这种设计使代理能够在稳定且确定的界面上运行，同时允许您切换本地或远程浏览器以及配置文件。

__HEADING_0__CLI快速参考

所有命令都接受 `--browser-profile <name>` 来指定特定的配置文件。所有命令还接受 `--json` 以生成机器可读的输出（稳定的负载）。
...
- `upload` 和 `dialog` 是**准备**调用；在触发选择器或对话框的点击或按下之前先执行它们。
...
- `snapshot`：
  ...
  - 配置默认值（仅适用于工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"`，以便在调用者未指定模式时使用高效快照（请参阅 [网关配置](/gateway/configuration#browser-openclaw-managed-browser)）。
...
JSON 中的角色快照包括 `refs` 加上一个小的 `stats` 块（行数/字符数/引用/交互性），以便工具能够推断负载大小和密度。

## 状态与环境旋钮

这些在“让网站表现得像X”工作流中非常有用：
...
- `browser act` 使用快照 `ref`ID 来执行点击、输入、拖动或选择操作。
- `browser screenshot` 捕获像素（整个页面或特定元素的像素）。
- `browser` 接受：
  - `profile` 用于选择命名浏览器配置文件（openclaw、chrome 或远程 CDP）。
  - `target`（`sandbox` | `host` | `node`）用于指定浏览器所在的位置。
  - 在沙盒会话中，`target: "host"` 需要 `agents.defaults.sandbox.browser.allowHostControl=true`。
  - 如果未指定 `target`：在沙盒会话中，默认使用 `sandbox`；在非沙盒会话中，默认使用 `host`。
