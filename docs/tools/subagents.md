...
- 在不阻塞主运行流程的前提下，并行处理与“研究/长任务/慢速工具”相关的工作。
- 默认情况下保持子代理隔离——通过会话分离和可选沙箱来实现。
...
- `model?`（可选；覆盖子代理模型；无效值将被忽略，子代理将以默认模型运行，并在工具结果中发出警告）
...
- 在请求者聊天中发送 `/stop` 将中止请求者会话，并停止从中派生的所有活动子代理运行。

## 限制

- 子代理声明其行为属于“尽力而为”。如果网关重启，待处理的“宣布回”任务将丢失。
