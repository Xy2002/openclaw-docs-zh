...
使用 `session.identityLinks` 将提供商前缀的对等ID映射到规范身份，以便在使用 `per-peer`、`per-channel-peer` 或 `per-account-channel-peer` 时，同一个人在不同渠道之间共享同一个直接消息会话。

## 网关是事实来源

所有会话状态均由**网关**（“主”OpenClaw）管理。UI客户端（macOS应用、WebChat等）必须向网关查询会话列表和令牌计数，而不能直接读取本地文件。
...
当会话接近自动压缩时，OpenClaw可以执行一次“无声内存刷新”，提醒模型将持久化笔记写入磁盘。此操作仅在工作区可写时运行。请参阅[内存](/concepts/memory)和[压缩](/concepts/compaction)。
...
- 为主密钥保留一对一流量；让每个群组各自保留其专属密钥。
- 在执行自动化清理时，应删除单个密钥，而不是清空整个存储，以保留其他地方的上下文。
