...
OpenClaw 将**每位代理的一个直接聊天会话**视为主要会话。直接聊天会折叠为 `agent:<agentId>:<mainKey>`（默认 `main`），而群组或频道聊天则拥有各自的密钥。`session.mainKey` 会被尊重。
...
提醒模型将持久化笔记写入磁盘。此操作仅在工作区可写时运行。请参阅 [Memory](/concepts/memory) 和 [Compaction](/concepts/compaction)。
...
## 生命周期
- 重置策略：会话会重复使用，直到过期，过期将在下一个入站消息时评估。
