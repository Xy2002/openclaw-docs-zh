...
想要“群组只能看到文件夹 X”，而不是“无法访问主机”？保留 `workspaceAccess: "none"`，只将白名单路径挂载到沙盒中：
...
- Discord 的默认设置位于 `channels.discord.guilds."*"`（可按公会/频道覆盖）。
...
- 某些通道对房间/频道的嵌套结构有所不同（例如，Discord `guilds.*.channels.*`、Slack `channels.*`、MS Teams `teams.*.channels.*`）。
