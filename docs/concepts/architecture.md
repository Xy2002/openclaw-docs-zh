...
- 每台主机仅运行一个 Gateway；它是唯一用于建立和维护 WhatsApp 会话的地方。
- 一个 **画布主机**（默认为 `18793`）提供可由代理编辑的 HTML 和 A2UI。
...
- 静态UI使用网关的WS API获取聊天历史并发送消息。
- 在远程设置中，通过与其他客户端相同的SSH/Tailscale隧道进行连接。
...
- 对于具有副作用的方法（`send`、`agent`），需要使用幂等密钥以实现安全重试；服务器维护一个短期去重缓存。
...
- **本地**连接（环回地址或网关主机自身的 tailnet 地址）可以自动批准，以确保同一主机上的用户体验顺畅无阻。
- **非本地**连接必须签署 `connect.challenge` 随机数，并需经过明确批准。
- 网关身份验证(`gateway.auth.*`)仍然适用于**所有**连接，无论其是本地连接还是远程连接。
...
- 同样的握手和身份验证令牌适用于隧道连接。
- 在远程设置中，可以为WS启用TLS，并选择性启用证书固定。

## 运维快照

- 启动：`openclaw gateway`（在前台运行，日志输出到 stdout）。
- 健康检查：通过 WS 提供 `health`（也包含在 `hello-ok` 中）。
- 监督机制：使用 launchd/systemd 实现自动重启。

## 不变性

- 每台主机恰好有一台网关负责控制一个Baileys会话。
- 握手是强制性的；任何非JSON或非连接的第一帧都会导致硬关闭。
- 事件不会被重放；客户端必须在出现间隙时执行刷新操作。
