...
由会话路由器自动固定的配置文件被视为一种 **偏好**：它们会首先被尝试，但 OpenClaw 可能在遇到速率限制或超时时切换到其他配置文件。用户固定的配置文件则始终锁定在该配置文件上；如果该配置文件失败且已配置模型回退，OpenClaw 会直接切换到下一个模型，而不会切换配置文件。
...
计费或信用失败（例如“积分不足”或“信用余额过低”）被视为可触发故障转移的情况，但通常不是暂时性的。与短暂的冷却期不同，OpenClaw 会将该配置文件标记为 **已禁用**（并采用更长的退避时间），然后切换到下一个配置文件或提供商。
...
当运行以模型覆盖（通过挂钩或 CLI）开始时，即使尝试了所有已配置的回退，回退最终仍会在 `agents.defaults.model.primary` 处停止。
