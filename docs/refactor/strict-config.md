...
---
# 严格配置验证（仅通过 Doctor 迈入迁移）

## 目标
- **在任何层级拒绝未知配置键**（包括根级别和嵌套级别）。
- **拒绝未定义模式的插件配置**；此类插件将不会被加载。
- **移除加载时的旧版自动迁移逻辑**；所有迁移操作仅通过 Doctor 执行。
- **在启动时自动运行 Doctor（以干运行模式执行）**；若配置无效，则阻止非诊断性命令的执行。
...
## 严格验证规则
- 每一层级的配置都必须与模式完全匹配。
- 未知键被视为验证错误（无论在根级别还是嵌套级别，均不允许透传）。
- `plugins.entries.<id>.config` 必须由插件的模式进行验证。
  - 如果插件缺少模式，**拒绝加载该插件**并显示明确的错误信息。
- 未知的 `channels.<id>` 键被视为错误，除非插件清单中已声明该通道 ID。
- 所有插件必须提供插件清单 (`openclaw.plugin.json`)。
...
- 插件加载流程：
  ...
  2) 使用模式验证配置。
  3) 若缺少模式或配置无效：阻止插件加载，并记录错误。
- 错误信息包括：
  - 插件 ID
  - 原因（缺少模式 / 配置无效）
  - 验证失败的具体路径
- 已禁用的插件仍保留其配置，但 Doctor 和日志会输出警告。

## Doctor 流程
- 每次加载配置时都会运行 Doctor（默认为干运行）。
- 若配置无效：
  - 输出摘要及可操作的错误信息。
  - 提示用户：`openclaw doctor --fix`。
- `openclaw doctor --fix`：
  - 应用迁移。
  - 移除未知键。
  - 写入更新后的配置。
...
所有其他命令必须硬性失败，并显示消息：“配置无效。请运行 `openclaw doctor --fix`。”
...
- `src/config/legacy*.ts`：将使用权限转移至仅限 Doctor。
- `src/plugins/*`：添加模式注册表 + 命令限制机制。
- CLI 命令限制在 `src/cli` 中实现。

## 测试
- 未知键拒绝（根级别和嵌套级别）。
- 插件缺少模式 → 插件加载被阻止，并显示明确的错误信息。
- 配置无效 → 启动时除诊断性命令外的所有命令均被阻止。
