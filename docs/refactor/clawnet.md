...
- 角色清晰：节点与操作员。
- 中央审批路由到用户所在位置。
- 所有远程流量均采用 TLS 加密，并可选固定。
- 最小化代码重复。
- 单一设备应仅显示一次（避免 UI/节点条目重复）。

## 非目标（明确说明）
- 不取消能力隔离（仍需遵循最小权限原则）。
- 不在未进行作用域检查的情况下暴露完整的网关控制平面。
- 认证不依赖于人类标签（别名与安全性无关）。
...
- 节诺将 `invoke-res` 返回给网关。
- 多跳路由，UI 与节点主机绑定。

## 当前存在感与身份
- 网关存在条目来自 WS 客户ент。
- 节诺存在条目来自桥接协议。
- macOS 应用可能为同一台机器显示两个条目（UI + 节诺）。
- 节诺身份存储在配对存储中；UI 身份独立。
...
- 需维护两套协议栈（WS + 桥接）。
- 遥远节点上的审批：提示出现在节点主机上，而非用户所在位置。
- TLS 固定仅存在于桥接协议中；WS 依赖 SSH/Tailscale。
- 身份重复：同一台机器显示为多个实例。
...
- 无法调用配置/模型/通道/会话/代理控制平面 API。

**操作员**
- 全功能控制平面 API，受作用域限制。
- 接收所有审批。
- 不直接执行 OS 操作；将任务路由至节点。
...
- 短暂的承载式令牌仅作为临时阶段（提前轮换 + 吊销）。

## 静默批准（SSH 启发式）
精确定义以避免成为薄弱环节。优先选择以下方式之一：
- **仅限本地**：当客户端通过环回/Unix 套接字连接时自动配对。
- **通过 SSH 挑战**：网关发出随机数；客户端通过获取该随机数来证明 SSH。
...
## macOS 应用
- 默认为操作员角色（控制 UI）。
- 当启用“Mac 节诺”时为节点角色（系统运行、屏幕、摄像头）。
- 同一 deviceId 对应两个连接 → UI 条目合并。
...
- 角尔/白名单在网关边界强制执行。
- 任何 clientent在没有操作员作用域的情况下都无法获得“完整”API。
- *所有*连接都需要配对。
- TLS + 固定降低了移动端的中间人攻击风险。
- SSH 静默批准是一种便利措施；但仍会被记录并可撤销。
- 发现服务绝非信任锚点。
- 功能声明由平台/类型根据服务器白名单进行验证。

# 流媒体 + 大型负载（节点媒体）
WS 控制平面适用于小型消息，但节点还处理：
- 摄像头片段
- 屏幕录制
- 音频流
...
3) 令牌轮换 + 吊销的用户体验
   - 在角色变化时自动轮换。
   - 提供按 deviceId + 角尔吊销的 UI。

4) 发现服务
   - 扩展现有的 Bonjour TXT，加入 WS TLS 指纹 + 角尔提示。
   - 仅将其视为定位提示。
...
- 当前：WS 控制平面 + 桥接节点传输。
- 痛点：审批、重复、双协议栈。
- 提案：单一 WS 协议，具有明确的角色与作用域；统一配对与 TLS 固定；网关托管审批；稳定设备 ID + 可爱别名。
