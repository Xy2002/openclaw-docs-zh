...
目标：每个消息传递连接器都作为一个插件（内置或外部）使用单一稳定的 API。任何插件都不直接从 `src/**` 导入。所有依赖项必须通过 SDK 或运行时来解决。

## 为何现在进行重构
- 当前的连接器混合了多种模式：直接导入核心、仅提供 dist 的桥接模块以及自定义辅助工具。
- 这种情况导致升级脆弱，并阻碍形成干净的外部插件接口。
...
- 先迁移 BlueBubbles、Zalo 和 Zalo Personal（这些已经接近目标）。
- 移除重复的桥接代码。
...
- 运行时：随核心版本进行版本化管理，并添加 `api.runtime.version`。
