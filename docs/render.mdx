...
如果您使用的是原始 OpenClaw 仓库，Render 不会自动部署您的 OpenClaw。要更新它，需从仪表板手动同步蓝图。
...
对于 OpenClaw，垂直扩展通常已足够。水平扩展需要粘性会话或外部状态管理。
...
免费层级的服务在空闲 15 分钟后会自动关闭。在关闭后首次请求时，容器启动需要几秒钟。升级到 Starter 方案可实现始终在线。
