...
其他网关斜杠命令（例如 `/context`)会转发到网关，并作为系统输出显示。请参阅 [斜杠命令](/tools/slash-commands)。
...
- `disconnected`：确保网关正在运行，并且您的 `--url/--token/--password` 配置正确。
